...
`gameModes.transitions` 保存显式有向 overlay edge。每条边拥有一套 official Spine 4.3 resource、
一个 once animation、该 animation 时间轴中恰好出现一次的大小写精确 switch event，以及恰好覆盖
active variants 的 art-space placement。没有直接边时 `requestGameMode()` 在任何可见 mutation 前失败；
...
`requestGameMode()` 在任何可见 mutation 前准备并严格验证目标 symbols scene、directed overlay
resource、once animation、唯一 switch event 和当前 variant placement。overlay 挂入固定
...
symbol package 与 active background nodes；`transitionPhase` 精确区分 `before-switch` 和
`after-switch`。
