...
BaseGame、FreeGame、BG 或 FG。每个模式为每个 active variant 显式绑定一个背景；
`nodeStates` 覆盖该模式的 stateful 背景和所有共享 stateful 普通 node。initial mode 的背景
必须逐项等于 adaptation bootstrap background，状态必须等于 node `initialState`。
同一 variant 的多个模式可以引用同一个 stateful Spine 背景 node，并通过各自 `nodeStates`
选择稳定动画；这类 BG/FG 表现不需要复制 node。模式切换使用该 node 的直接有向 transition，
once 完成后才提交目标稳定状态与模式。不同背景 resource 才使用不同 node，并在提交边界原子显隐。
...
导入旧 ZIP 时迁移为显式 `BaseGame`，background 从 adaptation 显式复制，stateful node 值
逐项复制 manifest `initialState`；legacy singular symbols binding 会升级为一项 library 并
绑定全部模式，再次导出只写 canonical plural 形式。
...
`requestGameMode()` 在任何可见 mutation 前准备并严格验证目标 symbols scene，再 preflight
全部 shared node direct transition；并行 transition 完成后在同一提交边界切背景显隐、替换或
移除 reel，并更新 stable mode。相同 symbols binding 禁止重复 target input 且保留现有 reel；
binding 改变且目标有 symbols 时必须提交目标 `reels.main`；目标无 symbols 时禁止提交。
默认会复用相同 binding 的现有 reel、scene 与 player；诊断或明确重建需求可传
`recreateReel: true` 和新的 `reels.main`，此时也只在 mode 提交边界替换并随后销毁旧 reel。
transition 中、popup active 时或并发请求都会失败。`getGameModeSnapshot()` 返回
`stableMode/targetMode/phase/stableSymbolPackage/targetSymbolPackage/activeBackgroundNodes`。
