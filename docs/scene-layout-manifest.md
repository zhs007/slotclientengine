...
binding `type` 必须与 nested popup manifest 精确一致。`order` 是 Popup root 的安全整数显示顺序，必须与全部 node、main reel 和其它 Popup order 唯一，并高于全部 node/main reel；旧单 Popup v1 缺省时规范化为 `2000`，多个缺省值造成重复时显式失败。顶层 `popups` 本身就是可由程序访问的导出目录，因此 binding 不要求一定被 game mode 或 transition 引用；未进入该目录的已导入 package 仍不可访问。game mode 的 `awardCelebrationPopup` 只能引用 `award-celebration`，transition 的 `preludePopup` 只能引用普通 Spine，`single-state` 只能由程序打开。三类程序调用统一使用 `openPopup()` / `closePopup()` 和 exact `gamelayout:/popup/<id>` owner 地址；同一 runtime 任一时刻只允许一个 active Popup。package runtime 先保持 source mode 播放 prelude popup；宿主通过 `bindPopupInput()` 绑定完整 canvas 与 keyboard target，runtime 的统一主操作锁存结束请求，popup 完整到达 complete 后才继续当前转场效果。idle 输入透传，未迁移 consumer 可继续使用 Pixi pointer fallback。
