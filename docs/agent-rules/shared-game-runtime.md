...
- Scene Layout 顶层`popups`是三类Popup的programmatic导出目录，不要求mode/transition直接引用。production统一通过exact owner address调用`openPopup/closePopup`；package runtime只允许一个active Popup，programmatic、mode award和transition prelude互斥，冲突在player mutation前失败且不自动替换、排队或叠加。每个binding只缓存一个package-owned player，关闭后复用，caller不持有或destroy raw player。
