...
- authored UI control 只能通过 `getUiControl(exactId)` 或 exact `gamelayout:/ui-control/<id>` endpoint 取得 borrowed discriminated capability，不得伪装为 RenderObject 或公开 Sprite/Texture。radio 的 pointer/programmatic mutation 共用一次 committed state path；初始化、same-state、失败和 destroy 不派发，真实改变只派发对应 state-specific entered event。
