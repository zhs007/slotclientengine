...
- Scene Layout package resource 必须在 RenderCore 内把合法 v1–v7 直接规范化为 latest v7，并为旧版本确定性生成缺失的 `runtimeAllocation`、普通图层方向 placement、空旧音频目录与默认 event-audio 合同；不得要求先经 Editor 重导。package runtime 只执行 strict latest runtime manifest，allocation 与 typed mode/node/Symbols/Popup/transition/runtime-resource/audio/event-address 引用不一致时显式失败，Editor 只导出完整 v7。
