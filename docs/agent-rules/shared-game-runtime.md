...
- value presentation 使用 strict `font | image | image-string` union；image-string 的 dependency、slot、transform、颜色跟随和特殊值图片按 Spine tier index 独立绑定，由 rendercore 在解析 value tier 后精确选择，不从首档、相邻档或共享字段 fallback；Spine slot attach 通过外层 wrapper 跟随 bone matrix，内部 display 保留自身 offset/scale/pivot。
...
- 普通 Spine popup prompt 的单行校验、按 area 等比缩小、缺省 `system-ui/sans-serif`、显式 package 字体 FontFace hash 复用/释放及 image/Spine/VNI overlay 生命周期属于 rendercore；系统字体不建模为 package 资源，游戏只向 `start(text?)` 传入已翻译 string，省略时使用 manifest 默认值。
