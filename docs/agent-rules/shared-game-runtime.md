...
- award win-amount 进入 `awaiting-dismiss` 后保留最后实际到达档位的 loop，玩家 advance 幂等且不再触发 end/close；该状态不得继续阻塞 `playSpin()`，下一次 spin 在 reel mutation 前负责立即清理遗留展示。显式 `requestDismiss()` 仍可用于 owner 请求正式 end/drain。
