...
- legacy grid-cell immediate stop 只接受已有 authoritative target 的 active target-aware spin：同步提交
  ...
  spin-only 表现，并通过既有 landing edge/appear completion 合同完成。targetless continuous 不得猜测落点，
  standard、idle、dropdown、effect sweep 与 refill/cascade 不得借该入口跳过；网络等待期的 one-shot 请求与
  refill eligibility 由 app activity gate 管理。
...
- Scene Layout symbol 状态 batch 与逐 occurrence symbol-state 是两个 event family。batch 每个 request 只在全部 request 完成预检后发一个不含坐标的 event，address identity 为 exact Symbols binding、代表 symbol 与 state；显式代表 symbol 必须存在且出现在该 request positions 中，省略时按 positions 内最小 symbol code 解析。全部 batch event 按 request 顺序发出后才允许启动逐 symbol mutation，失败或预先 abort 不得产生 event 或部分状态切换。
...
- symbol内置`valuePresentation.text.type: image-string`需要改变最终文字时，由app在package/runtime装配时显式注册`symbol -> synchronous formatter`；raw value仍唯一负责tier选择、occurrence carry和resolver，formatter string只进入settled、landing与lightweight rolling ImgNumber。该formatter不适用于font/image，不进入manifest，不得硬编码symbol、slot、除数、前缀或业务舍入到rendercore；返回值必须在画面mutation前通过non-empty string与当前tier glyph/special exact closure校验。
...
- `packages/uiframeworks`/`gameframeworks` 拥有 DOM frame、canvas 逻辑尺寸上限、黑边居中和 viewport resize policy；`framePolicy` 是必填的唯一 frame 尺寸输入，不保留独立 `designSize`、fixed policy 或隐式固定尺寸 fallback；app 不以私有 CSS/DOM resize 绕过。
- background、symbol 和 popup 复用官方 runtime、manual update、completion 和 destroy 底层；不得增加静态图、首帧或默认 animation fallback。

## Presentation

- rendercore 拥有通用 symbol win carousel、金额递增、big/super/mega tier、segmented VNI 播放、popup threshold sequence、完整 canvas/keyboard 输入绑定、advance/dismiss/end drain，以及普通 Spine popup 的 start→loop→end 状态机。Award 金额不滚动 base：小于等于 bet 直接 final/end，大于 bet 从 standard 的 exact bet 起跳；每档以完整 canonical threshold span 和 manifest duration 标定 nominal rate，partial final 不重新拉满时长。Core 在 start 时生成常量规模的跨档连续加速轨迹，分段模式只有 actual final 前使用一次 terminal braking；megawin 开放区间沿用最近封闭 celebration span 标定。Mega VNI 全部为 once 时，只有 Mega 数字段按实际剩余金额拟合项目有效秒数，速度连续且不得低于入档速度；无法同时满足时长和速度下限时提前完成，不运行减速 tail。Award advance 在 final 前跳到下一个实际可达 big/super/mega threshold，没有非最终档时进入同一 braking tail，Mega once 则直接提交 final 并正常收尾；庆祝档仍在 start 时也必须单击立即生效。共享 ImgNumber、formatted amount 与 active tier 必须在同一同步边界提交，不能出现新档位配旧金额。Popup 固定为 `data → core → editor`：gameframeworks、Scene Layout 与游戏只依赖 data/core，mapped package与完整snapshot只属于editor wrapper；所有 consumer 默认把任一受支持 source version strict规范化为latest，未知未来版本失败。每个领域只维护一份 canonical runtime state：游戏 Runtime 只开放命令、标量 query 与 edge drain，完整 snapshot 只由复用同一 Runtime 的 editor/diagnostic wrapper 生成；不得为 editor 与 game 复制状态机或把 mutable hot-path buffer 暴露为 public API。普通 Spine Popup 的 start点击忽略且不锁存，loop点击立即进入end，end点击忽略。Scene Layout transition prelude 只能编排 public Runtime；app/editor 只提供宿主 input target，不复制点击分派或状态边界。
- Award per-play正有限amount duration scale只等比缩放数字计数与terminal braking时间，不缩放VNI/Spine/粒子/音频或最终最低停留；Scene Layout play调用必须捕获本次raw amount formatter，FIFO请求和缓存player之间不得串值。自然计数和玩家advance到threshold都当帧隐藏旧档并显示新档，旧档只在不可见状态继续official end/update/drain，动画completion不得阻塞数字跨档。
- Popup player 是主 Pixi display tree 中的普通 Container 节点；rendercore 不为 Popup 创建独立 Application、canvas、Renderer、ticker 或 RAF。只有 Popup Editor 的独立预览页面可创建自己的预览 canvas，再挂载同一个 player Container。Popup v2 保留有限 design viewport 适配；v3–v9 不保存 design viewport，由 shared 无界 maximized-focus 以 focus 几何中心按 page aspect 扩展 visible rect。全屏 backdrop 始终覆盖宿主 viewport，不随 authored content focus transform 缩放；v6–v9 award layer 由 containing tier 决定可见性。
- Popup Object 是 Popup v9 可引用的 opaque 子 Container，不是独立 Popup 状态机。immutable definition可复用，但每个实例必须拥有独立mutable layer runtime；对象内部name、order与attachment只在本地解析，不扁平进入宿主registry或attachment图。实例activation/update/stop/destroy完全由宿主award/Spine/single-state layer推进，不创建ticker、backdrop、input或completion边界。
- Scene Layout project-wide Tap info definition由package resource唯一拥有；每个同时具备Popup mount metadata的普通Spine player创建独立instance，并与既有children进入同一official slot/VNI text owner group的确定性尾部。instance在start/loop逐帧一次、end前停用，且不进入Popup object/string/address registry；runtime先销毁instance，package再释放共享definition。
- Popup v4–v9 Spine-slot attachment graph、exact slot prepare、DAG/per-parent order 与一 slot 一稳定 owner group 属于 rendercore；同 slot 多 child 按 layer order 排序并保留局部 transform，跨 slot 顺序继续由 skeleton draw order 决定。award tier 切换必须先撤销旧 attachment/可见提交再挂新状态，旧效果不得在新档背后显示。
...
- award win-amount 到达最终金额后必须立即进入 `dismissing`，保持最终金额，同时启动项目最低停留计时并播放最后实际到达档位的完整 end/once；停留和动画 drain 均完成后自动关闭，等待两者较长者，不得停在需要额外输入的 final hold。缺省时长由资源加载边界从 Mega VNI metadata 补齐，显式秒数优先，未知资源或非法时长显式失败；计数跨 final 的同帧剩余 delta 只消费一次。segmented 庆祝动画在数字到达 final 前必须保持 loop，不得提前进入 end；显式 `requestDismiss()` 同样先提交最终金额，再启动正式 end/drain。
...
- Scene Layout package resource 必须在 RenderCore 内把合法 v1–v8 直接规范化为 latest v8，并为旧版本确定性生成缺失的 `runtimeAllocation`、普通图层方向 placement、空旧音频目录、默认 event-audio 与 absent authored Splash 合同；不得要求先经 Editor 重导。配置 Splash 时首击同步发起 audio unlock 与 direct initial transition；未配置时 RenderCore 的无资源黑色门禁强制首击成功 unlock 后才揭示 initial，app 不得恢复 first-spin unlock 第二流程。
- Scene Layout `json` runtime resource 是 app 配置的可选并行数据源，不替代 Symbols `gameConfig`。gameconfig 路径保持可用；app 必须显式选择 source，选择 layout data 时按 exact key 调用 `loadJsonData()` 并用 app-owned strict parser 验证轮带/权重业务语义。共享层不得按文件存在性、同名 key 或失败结果自动覆盖/fallback。
- AudioCore event track 的 music/effect 分类只路由玩家独立音量；focus 必须使用 owner-scoped per-voice lease 叠加，不得临时改写或恢复全局玩家 volume。lease owner 自身排除，重叠取最小 gain，并在 ended/stopped/failed/destroy 清理。
- AudioCore 是浏览器前后台音频生命周期的唯一 owner：backend 必须把 focus、visibility 与 page lifecycle 合成为 `active | suspended`；suspended 时取消且不补播 once，loop/BGM 仅暂停并保留仍有效 intent，host-clock delay/focus/crossfade 冻结。activity pause 与 BGM focus pause 必须组合，Pixi 全局 auto-pause 只能由 owner-scoped lease 协调并在最后 owner 销毁后恢复；app 与游戏不得另建 visibility resume 或批量补播逻辑。
...
- `SceneLayoutPackageResource.loadRuntimeResource(key, kind)` 是包内程序资源的 typed async prepare 边界；同 key 并发请求复用同一 Promise，kind/未知 key 按 canonical runtime manifest 精确失败，lazy initial layout view 不作为程序键目录；`getLoadedRuntimeResource` 只返回已成功 prepare 的资源。`loadJsonData(key)` 是 JSON 的易用代理并返回 deep-frozen object/array；语法、UTF-8、root、finite number 或 destroy 失败不得缓存 value/bytes。`loadRuntimeResource(key, "audio")` 返回 package-owned URL 与 exact mediaType，可复用同路径 Event audio 已解析 URL；audio 程序键还必须派生同名 effect route与canonical audio-effect address，播放 voice/handle由AudioCore唯一拥有，stop-before-load或destroy后的late resolve不得起播。audio仍不创建RenderObject；package destroy必须撤销自有Object URL。
...
- authored UI control 只能通过 `getUiControl(exactId)` 或 exact `gamelayout:/ui-control/<id>` endpoint 取得 borrowed discriminated capability，不得伪装为 RenderObject 或公开 Pixi 对象。radio 与 step-slider 的 pointer/programmatic mutation 各自共用唯一 committed state path；slider 拖动只改临时位置并由 runtime manual clock 吸附完成后提交。初始化、过程、same-state、取消、失败和 destroy 不派发，真实改变只派发对应 state-specific entered event。
...
- main reel 的 `spin-lifecycle` event 必须来自实际 spin owner 的 cohort/primitive start 与 authoritative target landing commit：每种 Spin 的无坐标 `started/ended` 各表示整体开始与成功结束；standard ReelSpin 发布 exact axis 与 `x/*`，GridCell/CellSpin 发布 exact cell、列、行、全体通配，通配 listener 的 occurrence address/detail 仍保留实际坐标。`all-stopped` 紧随本批最后一个参与单元落定，整体 `ended` 再紧随其后，均不等待 landing appear/dimming；continuous settle、selective、direct/session、immediate stop 与宿主自行 update 的 injected grid-cell 均走同一 observer，cancel/reset/destroy/failure 不得伪造 stopped 或完成边界。
- grid-cell 落点 appear/显式 landing state 必须经过 reel symbol observer 派发 public grid occurrence；Symbol Package 中显式声明 `appear` animation capability 的 symbol 必须自动启用 landing appear，不另建 app 清单；从 Scene Layout runtime resource 构造的 pooled grid-cell Spine effect 必须复用该 resource 的唯一 animation lifecycle address，不得因 app typed factory 或 player pool 绕过 event-audio。
...
- delivery runtime 在 loading resource phase 完成 initial owner 后启动 background queue，`media` 优先于非 initial mode；mode request 复用同一 in-flight owner load并在 target commit 前形成 readiness gate。transition prelude 的确认点击必须先锁存并正常完成 Popup 动画，资源稍后 ready 时自动继续，video 复用同一次 trusted gesture unlock，不要求用户再次点击。
