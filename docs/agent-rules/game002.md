...
- `assets/crave` 的当前美术 files/bytes 是 game002 权威交付；app/runtime/build
  使用 trusted-art policy，不比对 map `sha256`/`byteLength`、不因未引用 entry/file
  阻断。实际引用的 logical key 仍必须路由到安全存在的 path，并通过对应资源
  parser/decoder；不猜测未声明资源。
