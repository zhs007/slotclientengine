...
- 正式外层项目使用 strict `gameviewer2-project` v3，并同时保存
  `scene-other-scene-flow` v2 presentation 与 `slot-operation-authoring-project` v1 evidence。
  外层 v2 只能显式升级为 `review: required` 草稿；未完成 review/final closure 时不可
  preview/export，v1 继续拒绝。
...
- finalized plan 必须由 rendercore 实例级 operation coordinator 执行；settled scene/value
  只能从当前 `operation.output` 提交，不得在 checkpoint 校验后回到 snapshot 直通 mutation。
