...
- Gamelayout Editor 只编辑和导出 Scene Layout latest v6；打开合法 v1–v6 时先调用 RenderCore 共享 upgrader，确定性补齐 allocation、旧单背景普通图层的横竖 placement、空旧音频目录与默认 event-audio 合同，再执行 Editor node-id migration 并原子复验。每次导出都由 RenderCore 从 typed draft 重建并 strict 复验 allocation 与 event catalog 引用；旧 mode id、initial、edge、dependency 和旧音频数据保持，不自动插入 Splash。
