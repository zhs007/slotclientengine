...
- 新项目使用中心坐标且只创建 BaseGame initial；用户需要 authored Splash 时先创建普通 mode，再单独设为 Splash并显式配置 Splash→initial direct transition，同一 mode 不得兼任两种角色。未配置时 production/preview 使用 RenderCore 的纯黑默认 Splash 点击门禁；Editor 不保存或导出这个默认画面。
