...
- Popup v5 沿用 v4 的无界 focus、backdrop、layer alpha 与 strict attachment，并为 backdrop 和所有 layer kind 保存 type-aware `visibleStates`：award 使用五档，Spine Popup 使用三阶段。Popup Editor 新建项目固定为 v5；合法 v1–v4 ZIP 必须先按原版本 strict 校验与 prepare，再原子迁移为 v5，后续 preview/export 只写 v5。旧三阶段全选扩展为目标全选，部分选择按固定 index 迁移；rendercore 继续原样运行 v1–v4。
...
- 新 Popup v5 authoring 不提供独立 Spine prompt；旧 prompt 在 v1/v2 导入迁移时结构化转换为 `name=prompt` 的字体文字 overlay，冲突使整次导入失败。award 各档与 Spine overlay 都可声明多个命名字体文字和 manual ImgNumber；文字省略 resource 时使用系统字体，显式选择时只接受 package-owned WOFF2/WOFF/TTF/OTF，失效引用不降级。文字颜色色板/string、Curved Text、默认 string、transform/order/项目状态可见性及 ImgNumber 默认 string 只在 Popup Editor 编辑，每个 award 档仍必须恰好有一个 `win-amount`。游戏按 exact name 修改 string；Popup Editor preview 不提供临时 node set/reset。其它 image/Spine/VNI overlay 的 transform、order、项目状态可见性与 playback 同样只在 Popup Editor 编辑。
- Popup v4/v5 的 image、字体文字、ImgNumber、VNI 与 Spine layer 可挂 Popup root 或同作用域 official Spine exact slot；普通 Spine Popup 还可挂主 Spine，ImgNumber 保留 VNI text-layer attachment。Editor 候选必须来自 shared Spine/VNI strict metadata，不猜首项；循环 target、跨作用域引用、缺 slot、同父 order 冲突、覆盖后失效与删除被引用 target 都阻止 transaction，不自动回根。
