...
- Popup v4 沿用 v3 的无界 focus、backdrop 与 layer alpha，并为每层保存 strict attachment。Popup Editor 新建项目固定为 v4；合法 v1/v2/v3 ZIP 必须先按原版本 strict 校验与 prepare，再原子迁移为 v4，后续 preview/export 只写 v4。rendercore 继续原样运行 v1/v2/v3。
...
- 新 Popup v4 authoring 不提供独立 Spine prompt；旧 prompt 在 v1/v2 导入迁移时结构化转换为 `name=prompt` 的字体文字 overlay，冲突使整次导入失败。award 各档与 Spine overlay 都可声明多个命名字体文字和 manual ImgNumber；文字省略 resource 时使用系统字体，显式选择时只接受 package-owned WOFF2/WOFF/TTF/OTF，失效引用不降级。文字颜色色板/string、Curved Text、默认 string、transform/order/segment 及 ImgNumber 默认 string 只在 Popup Editor 编辑，每个 award 档仍必须恰好有一个 `win-amount`。游戏按 exact name 修改 string；Popup Editor preview 不提供临时 node set/reset。其它 image/Spine/VNI overlay 的 transform、order 与 playback 同样只在 Popup Editor 编辑。
- Popup v4 的 image、字体文字、ImgNumber、VNI 与 Spine layer 可挂 Popup root 或同作用域 official Spine exact slot；普通 Spine Popup 还可挂主 Spine，ImgNumber 保留 VNI text-layer attachment。Editor 候选必须来自 shared Spine/VNI strict metadata，不猜首项；循环 target、跨作用域引用、缺 slot、同父 order 冲突、覆盖后失效与删除被引用 target 都阻止 transaction，不自动回根。
