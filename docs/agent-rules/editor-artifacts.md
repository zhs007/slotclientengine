...
- Popup v3 不保存有限 `designViewport`，只保存相对 Popup 原点的正有限 `focus.left/right/top/bottom`、默认启用的 50% 黑色全 viewport backdrop，以及每层 `alpha`；runtime 在无界 authored plane 上 contain focus 并按宿主 page aspect 向外扩展 visible rect。Popup Editor 新建项目固定为 v3；合法 v1/v2 ZIP 必须先按原版本 strict 校验与 prepare，再原子迁移为 v3，后续 preview/export 只写 v3。rendercore 继续原样运行 v1/v2。
