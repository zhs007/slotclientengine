...
- Popup Editor 的 award tier VNI 支持 `segmented` 与 `once`，两种 playback 字段互斥；segmented 必须显式保存合法 loop 边界。项目级 Mega once 有效计数与最终停留配置保存秒数，复用 RenderCore 的 metadata 默认解析；缺省加载补齐，已有值优先，不在 Editor 复制金额或收尾状态机。
...
- Game Layout Editor 可严格导入 standalone Popup Object ZIP 到候选库，并在项目级显式选择零或一个 Tap info 对象；导入不得自动绑定，未选对象不得进入production closure，删除已选对象必须先清空binding。同name替换与失败回滚保持旧selection和bytes。项目 binding 本身不得在 default/authored Splash 自动显示对象；绑定对象同时作为只读 Popup Object 图层资源，用户可显式加入 Layout，默认从 Popup order 2000 起分配，并手工配置中心坐标 placement 与 Splash mode/orientation scope。声明 exact attachment 的普通 Spine Popup 仍可使用自己的独立实例。
...
- Gamelayout Editor 只编辑和导出 Scene Layout latest v8；打开合法 v1–v8 时先调用 RenderCore 共享 upgrader，把 legacy 坐标、main 与 per-mode background binding 确定性规范化为中心坐标、普通 scoped node 和 allocation v3，再执行 Editor node-id migration并原子复验。每次导出都由 RenderCore 从 typed draft 重建并 strict 复验 allocation、Splash role/direct edge 与 event catalog 引用；旧 mode id、initial、edge 和只读 dependency 保持，不按名称推断或自动插入 authored Splash。root legacy music/effect 与 mode BGM 只作为 strict import 输入，迁移后从 draft 与仅由其拥有的 bytes 中移除并显示摘要；与 Event audio 共用的 asset 必须保留。
...
- 新项目使用中心坐标且只创建 BaseGame initial；用户需要 authored Splash 时先创建普通 mode；“设为 Splash”只保存 role，不自动创建转场。Splash→initial 未配置转场时直接切换，已有转场按配置执行。不同的 primary action target 必须先显式调整，不得静默改写。同一 mode 不得兼任 initial 与 Splash。未配置时 production 使用 RenderCore 的纯黑默认 Splash 点击门禁；Editor 通过 authoring mode selection 直接显示所选状态，不显示默认 Splash，也不自动解锁声音。Editor 不保存或导出这个默认画面。
- mode 的主转轮开关属于 v7–v8 draft；关闭时仍保留横竖 main/focus authoring geometry但不绑定 Symbols，已有 Symbols binding 必须先显式解除。
...
- gamelayouteditor 的普通 node `scope` 属于 v7–v8 layout manifest typed contract：字段缺失表示全局，字段存在时精确声明 mode × orientation；legacy 单 `gameMode` 与 per-mode background binding 只在共享 upgrader 中转成 canonical scope。Editor draft 只保存 scope，必须在结构化导入、编辑、导出、优化改写和重导中原样保留；不得把它存入 UI session、文件名约定或第二份资源表。scope/placement 隐藏不改变 node 的全局 order 或资源 identity，空 scope、unknown mode 与缺 placement 引用显式失败。
