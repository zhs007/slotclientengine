...
- Popup v2 保存中心基准的 `focus.left/right/top/bottom`、默认启用的 50% 黑色全 viewport backdrop，以及每层 `alpha`；focus 先使用与 Game Layout 相同的 `maximized-focus` 算法求 visible rect，再等比 contain 到 viewport 并居中留边。v1 导入、预览和原版本导出保持兼容，升级 v2 必须显式执行。
