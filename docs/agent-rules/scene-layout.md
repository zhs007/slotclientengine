...
- v7 `nodes` 是严格的 `resource | uiControl` 图层 union；v1–v6 只接受图形。UI 控件使用可扩展 discriminated union，当前 `radio` 必须显式绑定不同且同尺寸的 off/on image root，复用唯一 id/order/scope/placement，固定以 off 初始化；Editor 不按文件名配对，也不把控件降级为普通 image 图层。
