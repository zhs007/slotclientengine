...
- Editor 普通配置事务不得复制或扫描资源 payload；资源修改保留独立隔离事务。结构性编辑只标记预览未同步，由预览区“刷新预览”显式重建；首次加载/项目替换自动准备，已同步 runtime 的 geometry-only 更新继续实时复用。待刷新期间不得把旧 runtime 当作最新 draft 执行转场或打开 Popup，刷新中的新编辑仍须保留待刷新状态；导出始终读取当前 draft。
