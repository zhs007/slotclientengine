...
- 新 delivery 恒写 version 2：manifest 使用 `delivery.<full-sha256>.json`，metadata ZIP、atlas 与 external media 使用 `<full-sha256>.<canonical-ext>`，全部作为普通文件平铺在同一个 append-only CDN pool，不得生成 physical 子目录、unhashed alias 或 latest pointer。包内 metadata ZIP 与 `assets.map.json` 继续使用标准 mapped package path，不得从 CDN 扁平 filename 反推或改写 logical identity。
- delivery publisher 只新增缺失对象，已有同名文件必须 byte-equal，旧 hashed manifest/payload 不删除；所有 payload 必须先于 manifest 发布。`--check` 只要求当前 candidate closure 存在且 byte-equal，允许额外合法旧版本；GC、cache header、CORS 与 CDN purge 属于部署边界。
...
- RenderCore delivery loader 由调用方显式传 HTTP(S) directory `urlPrefix` 与 exact `manifestFilename`，所有manifest/payload URL只相对该prefix解析；部署prefix不进入manifest，禁止从JS/page URL推断。runtime按明确版本继续读取历史v1 nested delivery，新CLI只生成v2，不提供v1/v2路径fallback或旧参数alias。
