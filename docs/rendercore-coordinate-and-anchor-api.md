...
```ts
const valueAnchor = symbol.getValueAnchor();
const textAnchor = symbol.getTextAnchor("value");
```

- `getValueAnchor()` 指向 symbol 的实际 `valuePresentation`；
- `getTextAnchor(name)` 指向 exact named image-string node；
- 二者都不是整个 symbol 的中心；
- 未配置对应 presentation、名字未知或 occurrence stale 时显式失败。

通常分别配合 `cloneValue()` 和 `cloneText(name)`，用于只飞表现副本而不修改盘面值。
...
```ts
await scope.transfer(area.getLayer("win"), symbol.cloneValue(), {
  from: symbol.getValueAnchor(),
...
generic `move/transfer` 只移动临时 `RenderNode` 或 owned clone，不提交目标 value，也不改变盘面 occurrence。
