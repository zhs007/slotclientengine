...
- **Actions:**
  - Systematically refactor the following public and private methods to use `_enqueueOperation`:
    ...
    - `selectOptional()`
    - `collect()`
  - The core logic of each method (state checks, setting new state, calling `this.send()`) will be moved _inside_ the executor function passed to `_enqueueOperation`.
