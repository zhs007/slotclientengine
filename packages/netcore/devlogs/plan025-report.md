...
An early version of the fix had a flaw. It unconditionally set `curSpinParams` whenever a `gamemoduleinfo` with `finished: false` was received. This caused a regression by breaking the _normal_ spin-to-choice flow, where `curSpinParams` was already correctly set by the `spin()` method and was being overwritten with `undefined` values.
