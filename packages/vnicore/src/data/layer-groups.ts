...
type NormalizableVNIProjectConfig = Omit<VNIProjectConfig, "layerGroups"> & {
  layerGroups?: VNILayerGroupConfig[];
};
...
export function normalizeVNIProjectLayerGroups(
  project: NormalizableVNIProjectConfig,
): VNIProjectConfig {
...
export function normalizeV5GProjectLayerGroups(
  ...
  if (!hasLayerGroups) {
    const defaultGroup = createDefaultVniLayerGroup();
    return {
      ...project,
...
export function createDefaultVniLayerGroup(): VNILayerGroupConfig {
  return {
...
export function getV5GProjectRenderGroupOrder(
  ...
  for (const layer of project.layers) {
    ...
    const renderGroup = renderGroups.at(-1);
    if (!renderGroup) {
