...
- animation、particle、`particle_stream`、`chaser_light`、mask、project sampler，以及独立的 `safe_glow` 同图副本高亮 sampler。`chaser_light` 的灯位固定、弧长 spacing 和 `lightDuration + interval` 时序都属于 `vnicore` sampler 合同。
