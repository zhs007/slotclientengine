...
describe("particle_combo target variants", () => {
  ...
  it("uses a fixed duration and reports the changed effective speed", () => {
    const result = createVNIParticleComboTargetVariant({
    ...
    expect(getCombo(result.project).duration).toBe(2);
  });

  it("lists only enabled particle_combo animations with authored values", () => {
    const project = createProject();
...
function getCombo(project: VNIProjectConfig) {
  return project.layers[0].animations[0];
}

function createProject(): VNIProjectConfig {
  return {
