...
`safe_glow` 的 runtime 支持由 core/Pixi tests 和 `lock_01` / `roundreel` fixture 覆盖；它是继承 layer blendMode 的同图副本高亮，不属于 `shatter` / `glow` render effect 示例。`chaser_light` 的固定灯位、弧长 spacing 和亮灭推进由 core/Pixi tests 与 `roundreel` fixture 覆盖，示例层不复制走马灯算法。
