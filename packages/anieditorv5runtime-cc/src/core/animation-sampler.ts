...
export function isSupportedAnimationType(
  ...
  return SUPPORTED_ANIMATION_TYPES.indexOf(value as V5GAnimationType) >= 0;
}

export function isParticleAnimationType(
  value: string,
): value is V5GAnimationType {
  return PARTICLE_ANIMATION_TYPES.indexOf(value as V5GAnimationType) >= 0;
}
...
export function isDeterministicEffectAnimationType(
  ...
  return (
    DETERMINISTIC_EFFECT_ANIMATION_TYPES.indexOf(value as V5GAnimationType) >= 0
  );
}
...
export function isSupportedEasing(value: string): value is V5GEasingName {
  return SUPPORTED_EASINGS.indexOf(value as V5GEasingName) >= 0;
}
