...
创建后同步显示黑色 shell 和 logo；GIF load/error/timeout 后启动 intro gate，gate 完成再显示由 controller progress 驱动的 `a2/a3` reveal。装饰图片失败不会阻塞业务，controller 仍必须同时等待自己的资源和 live prepare。enter 成功后才播放 exit。
