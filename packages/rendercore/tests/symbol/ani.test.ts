...
function createContext(
  state = "normal",
  options: Pick<
    Partial<SymbolAnimationContext>,
    "stateTextures" | "requiredStateTextures"
  > = {},
...
describe("ManualSymbolAni", () => {
  ...
  it("reports loop and once completion as edge events", () => {
    ...
    const once = new ManualSymbolAni({
      stateId: "once",
      playback: "once",
      durationSeconds: 1,
    });
    expect(once.update(0.99)).toEqual({
      loopCompleted: false,
      onceCompleted: false,
    });
    expect(once.update(0.02)).toEqual({
      loopCompleted: false,
      onceCompleted: true,
    });
    expect(once.update(1)).toEqual({
      loopCompleted: false,
      onceCompleted: false,
    });
  });
...
describe("default viewer ani factories", () => {
  it("resolves state textures with fail-fast required texture checks", () => {
    ...
    expect(() => resolveSymbolTextureForState(context, "disabled")).toThrow(
      SymbolAssetError,
    );
  });
  ...
  it("uses requested state texture when resetting static display", () => {
    ...
    context.underlayLayer.addChild(new Sprite(Texture.WHITE));
    const ani = createStaticSymbolAni(context);

    ani.reset();

    expect(context.underlayLayer.children.length).toBe(0);
  ...
  it("uses a state texture for the win shine overlay when one is configured", () => {
    ...
    expect((context.overlayLayer.children[0] as Sprite).texture).toBe(
      winTexture,
    );
  });
});
