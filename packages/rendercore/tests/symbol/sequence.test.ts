...
describe("SymbolStateSequenceController", () => {
  it("advances stable states by holdSeconds and once states by completion", () => {
    ...
    expect(controller.update({ deltaSeconds: 0.49 })).toMatchObject({ shouldRequestState: false });
    expect(controller.update({ deltaSeconds: 0.01 })).toMatchObject({
      shouldRequestState: true,
      state: "appear",
      currentIndex: 1
    });
    expect(controller.update({ deltaSeconds: 10, onceCompleted: false })).toMatchObject({
      shouldRequestState: false,
      state: "appear"
    });
    expect(controller.update({ deltaSeconds: 0, onceCompleted: true })).toMatchObject({
      shouldRequestState: true,
      state: "normal",
      currentIndex: 0
    });
  ...
  it("supports add, remove and move while keeping index valid", () => {
    ...
    expect(controller.getSteps().map((step) => step.state)).toEqual(["normal", "disabled", "win"]);

    controller.moveStep(2, 0);
    expect(controller.getSteps().map((step) => step.state)).toEqual(["win", "normal", "disabled"]);

    controller.removeStep(1);
    expect(controller.getSteps().map((step) => step.state)).toEqual(["win", "disabled"]);
    expect(controller.update({ deltaSeconds: 10, onceCompleted: true }).shouldRequestState).toBe(false);
  });
  ...
  it("can play, pause, reset and request next manually", () => {
    ...
    controller.pause();
    expect(controller.update({ deltaSeconds: 100 }).shouldRequestState).toBe(false);
  });

  it("rejects invalid sequence data", () => {
    expect(() => new SymbolStateSequenceController({ statePreset: preset, steps: [] })).toThrow(
      SymbolStateError
    );
    expect(
      () =>
    ...
    expect(() => controller.addStep({ state: "normal" }, 0.5)).toThrow(SymbolStateError);
    expect(() => controller.updateStep(0, { state: "normal", holdSeconds: Number.NaN })).toThrow(
      SymbolStateError
    );
  });
});
