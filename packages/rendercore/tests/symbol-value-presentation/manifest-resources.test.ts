...
const TEST_SPINE_SKELETON = {
  ...
  bones: [{ name: "root" }],
  slots: [{ name: "ValueSlot", bone: "root" }],
...
describe("symbol value presentation manifest resources", () => {
  ...
  it("parses exact materialized full-value image mappings", () => {
    const mapped = createGenericManifest(1);
    const presentation = mapped.symbols.GOLD.valuePresentation;
    presentation.text = {
      type: "image",
      slot: "Num",
      x: 0,
    ...
    const parsed =
      parseSymbolStateTextureManifest(mapped).symbols.GOLD.valuePresentation!;
    expect(parsed.text.type).toBe("image");
    if (parsed.text.type !== "image") throw new Error("expected image text");
    expect(createSymbolValuePresentationImagePath(parsed.text, 10)).toBe(
      "./assets/10.png",
    );

    delete presentation.text.images["10"];
    expect(() => parseSymbolStateTextureManifest(mapped)).toThrow(
      /exactly match defaultValues/,
    );
    presentation.text.images["10"] = "./assets/10.png";
  ...
  it("binds a declared value texture to the atlas page", () => {
    const manifest = createGenericManifest(1);
    manifest.symbols.GOLD.valuePresentation.tiers[0].animation.texture =
      "./content-addressed-value.webp";
    const resources = createSymbolValuePresentationResourcesFromManifest({
      manifest,
      spineSkeletonModules: {
        "./tier-1-0.json": TEST_SPINE_SKELETON,
      },
      spineAtlasModules: { "./shared.atlas": TEST_SPINE_ATLAS },
      spineTextureModules: {
        "./content-addressed-value.webp": "/assets/physical-hash.webp",
      },
    });

    expect(resources.GOLD.tiers[0]).toMatchObject({
  ...
  it("fails when an image-rendered value has no exact image module", () => {
    ...
    expect(() =>
      createSymbolValuePresentationResourcesFromManifest({
        ...
        spineAtlasModules: { "./shared.atlas": TEST_SPINE_ATLAS },
        spineTextureModules: { "./shared.png": "/shared.png" },
        textImageModules: { "./1.png": "/1.png" },
      }),
    ).toThrow(/value image 10.*missing/i);
  });
  ...
  it("rejects a configured text slot missing from any tier skeleton", () => {
    expect(() =>
      createSymbolValuePresentationResourcesFromManifest({
        ...
        spineSkeletonModules: { "./tier.json": TEST_SPINE_SKELETON },
        spineAtlasModules: {
          "./Symbol.atlas": TEST_SPINE_ATLAS,
        },
  ...
  it("rejects empty/reversed/finally bounded tiers and non-Spine fallback", () => {
    for (const [index, mutate] of [
      (copy: any) => (copy.symbols.GOLD.valuePresentation.tiers = []),
      (copy: any) =>
        (copy.symbols.GOLD.valuePresentation.tiers[1].maxExclusive = 9),
      (copy: any) =>
        (copy.symbols.GOLD.valuePresentation.tiers[3].maxExclusive = 2000),
      (copy: any) =>
        (copy.symbols.GOLD.valuePresentation.tiers[0].animation.kind =
          "static"),
      (copy: any) => (copy.symbols.GOLD.normal = "./GOLD.png"),
      (copy: any) => (copy.symbols.GOLD.valuePresentation.defaultValues = []),
      (copy: any) =>
        (copy.symbols.GOLD.valuePresentation.defaultValues = [1, 1]),
      (copy: any) =>
        (copy.symbols.GOLD.valuePresentation.defaultValues = [0, 1]),
      (copy: any) => (copy.symbols.GOLD.animations.appear.playback.loop = true),
      (copy: any) =>
        (copy.symbols.GOLD.animations.appear = {
          kind: "static",
          durationSeconds: 1,
        }),
      (copy: any) => (copy.symbols.GOLD.valuePresentation.text.type = "video"),
      (copy: any) =>
        (copy.symbols.GOLD.valuePresentation.text.prefix = "../escape-"),
    ].entries()) {
      const copy = createGenericManifest(4);
      mutate(copy);
      expect(
