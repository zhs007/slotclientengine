...
grid-cell 可选 effect API 把逐格 presentation 与 symbol state 分离。`createGridCellEffectResourcesFromManifest()` 用精确 module maps 校验官方 Spine 4.3 skeleton、atlas page/texture closure、大小写精确 animation 和官方 duration；`GridCellEffectController` 在 prepare 阶段按 schedule-derived capacity 初始化有界 player pool，并用一个完整 grid rect mask 承载 cell-center overlay。spin plan 可为每格声明 effect lead 和可选 activation gate；runtime 只在真实 gate landing edge 后开放 gated effect，并按时间边界切片，真实 loop completion 先于同边界 landing。`startEffectSweep()` 支持已排序 position 副本和 stagger，等待最后一个真实 loop edge；reset/error/destroy 会清理并归还全部 player，不用 timer、symbol state 或 app 私有 Spine track。
