...
Award Popup 的 `requestAdvance()` 按实际可达里程碑跳档。尚未进入 bigwin 时，低于 bigwin 的获奖一次跳到最终金额；达到 bigwin 或更高时，一次点击把共享 ImgNumber 和画面同步跳到 bigwin 阈值，之后每次点击只进入下一个实际可达的 superwin/megawin，或在没有下一档时跳到最终金额。到达最终金额后立即进入 `dismissing`，播放最后档 end 并在 drain 完成后自动关闭。调用方也可用 `requestDismiss()` 提前提交最终金额并启动同一 end lifecycle，或用 `dismissImmediately()` 同步清理。
