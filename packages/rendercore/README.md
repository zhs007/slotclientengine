...
`@slotclientengine/rendercore/popup` 提供 strict `award-celebration | spine` popup parser、Popup id validator、typed filename-key namespace rewrite、传递资源闭包、files/CDN loader 与 snapshot。普通 Spine popup 使用独立 start→loop→end 状态机：点击请求可提前锁存，只在 loop 完整播放到边界后进入 end；可选 prompt 接收游戏传入的已翻译单行 string，缺省字体时使用 `system-ui, sans-serif` 且不产生 package 资源，显式 package-owned 字体时按配置区域自动缩小并按 bytes SHA-256 共享浏览器 FontFace owner，最后一个 package 销毁时释放；prompt 还可组合 image、命名系统文字、命名 ImgNumber、VNI 与 official Spine overlay。获奖庆祝支持同样的文字/ImgNumber 节点、五档 BigInt threshold sequence 与金额格式，并要求每档恰好一个 win-amount。Scene Layout 的每条 Spine transition 可独立用 `preludePopup` 复用该 player，并在 complete 后才启动 overlay；video transition 显式禁止 prelude。Popup binding 的 root `order` 与 node/main reel 全局唯一且必须更高，runtime 在当前 scene 的顶层 Popup root 内按该值排序。
...
symbol 可声明零到多个有唯一 name 的 `imageStringNodes`，每个节点引用 package 内的 standalone image-string 精确资源闭包并绑定一个或多个 state。official Spine/档位 activeSpine target 使用 exact `{state, slot}`；其他 visual kind 使用无 `slot` 的 `{state}`，renderer 挂在 symbol 固定顶层 ImgNumber overlay，composite 不选择内部 leaf。`createSymbolPackageResource()` 会递归校验 manifest、glyph、图片尺寸、initial text、Spine skeleton 与 slot；资源在 symbol 间共享，不使用宽泛 glob。

命名 node 与 `valuePresentation.text.type: "image-string"` 都支持可选 `specialValueImages: [{ value, image }]`。`value` 是唯一 safe integer，`image` 是 contained local 图片路径；完全匹配 `String(value)` 时整张 Sprite 替代 glyph renderer，其他字符串仍严格走 glyph closure。映射图片进入 package/Vite 精确闭包并与 glyph 纹理共享资源所有权，切换文本保持原 anchor、transform、target/slot 与 `followSlotColor`。
