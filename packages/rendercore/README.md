...
`step-slider` 用不同的 track/thumb 图片和稳定 Container，配置 `steps >= 2` 与正吸附时长；点击或拖动松开后由
runtime manual clock 吸附，完成时才提交数字档位并发布 `.../step-slider/state/<index>/entered`。其 capability 的
`setState(index)` 为可等待 Promise。
...
Popup v9 还可引用独立 `popup-object.manifest.json` v1。对象不是新的 Popup type：它只有 stable `name`、五类内部资源与 single-state 图层，没有 adaptation/backdrop/input/completion，并禁止对象嵌套。三类 Popup 都把对象实例作为 opaque Container；`getObject(id)` 返回局部 `getLayer/getTextNode/getImageStringNode` handle，不把 child name 合入宿主 registry。Popup latest 因此仍为 v9。
...
Popup v9 普通 Spine manifest 可选保存外部 Tap info 子对象的父节点 metadata。其窄 union 只接受 main Spine exact slot或当前 Spine overlay的VNI exact文字层；data层验证形状和overlay identity，core prepare再用official resource metadata验证目标。该字段不引用或实例化对象、不增加package资源闭包，也不进入Popup runtime display tree、ticker、input或destroy职责。
...
Scene Layout v8 继承的可选根 binding `tapInfoObject: { manifest }` 由 package resource 按 standalone Popup Object exact closure 准备并拥有，但 binding 本身不为 default/authored Splash 自动创建实例。直接显示必须声明 `{ kind: "popup-object", manifest }` Scene node，manifest 必须与根 binding 一致；该节点按普通中心坐标 placement、rotation center、mode/orientation scope 和 node lifecycle 渲染，并属于 Popup 级 order。普通 Spine Popup 只有在自身 v9 manifest 同时声明 `spine.tapInfoObject.attachment` 时才另建独立 mutable instance；instance 与已有 overlay 合并进同一 main Spine slot/VNI text-layer attachment transaction，并固定为同父尾部 child。Popup attached 对象随 Popup start/loop update、进入 end 前停用，不进入 `objects/getObject()`、string registry、runtime address，也不建立自己的 input/ticker/completion。
...
Scene Layout v8 的 `gameModes.splashMode` 是可选 authored 欢迎 mode，必须与正式 `initialMode` 不同并有 exact Splash→initial direct edge。配置时 `requestPrimaryGameModeAction()` 在同一真实手势、任何 await 前同时发起 AudioCore unlock 与现有 mode request；没有配置时 package runtime 在 initial 上方显示 full-viewport 纯黑默认 Splash，门禁期间拒绝直接 mode request，并在 primary click 成功 unlock 后才揭示 initial。Tap info Object 只在 manifest 含显式 Popup Object node 且当前 mode/orientation scope 命中时显示；默认黑画面本身不进入 manifest、allocation 或 delivery 资源，因此不能承载手工 Scene 图层。
...
停轴后，`RenderReelSet`（逐轴 spin）和 `RenderGridCellReelSet`（逐格 spin）都结构化实现 `VisibleSymbolPresentationTarget`：批量请求可见 symbol state、读取状态/几何快照并推进 animation。这个共同能力不合并两种 spin plan；grid-cell 的几何会把内部单行 reel 坐标转换为完整 grid 本地坐标，且 idle `update()` 会继续推进 `win once -> normal`。Scene Layout 的 `playMainReelSymbolStateBatch()` 还为每个 request 发出一个独立 `symbols-state-batch` event；request 可显式提供必须出现在 positions 中的 `symbol`，省略时按 positions 中最小 symbol code 解析。batch event 地址只含 binding、代表 symbol 和 state，不含坐标；既有逐 occurrence `symbol-state` entered/exited event 保持为另一事件族。
