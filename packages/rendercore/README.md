...
symbol manifest 可为任意 symbol 声明可选 `valuePresentation`。新 image-string wire 使用与 Spine tiers 等长的 `tierResources[]`，每档只保存 ImgNumber JSON；`slot`、anchor、transform、`followSlotColor` 与 special map 是一份共享 Normal 配置。旧完整 `text.tiers[]` 继续按原档位语义运行；shared 与 legacy 字段严格互斥。runtime 维持一个稳定外层 ImgNumber container：同档改值只 `setText()`，跨档替换内部 profile 而不改变外层 identity。
