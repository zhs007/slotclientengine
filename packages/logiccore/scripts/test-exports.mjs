...
assertTopLevelCore(importedCore.createGameConfig ? importedCore : importedCore.default, 'Node import');
...
function assertBuiltFile(relativePath) {
  ...
  if (!existsSync(filePath)) {
    throw new Error(`${relativePath} does not exist. Run pnpm build before pnpm test:exports.`);
  }
}
...
function assertTopLevelCore(api, label) {
  ...
  if ('loadGameConfigFromJsonFile' in api) {
    throw new Error(`${label}: loadGameConfigFromJsonFile leaked from top-level export.`);
  }
}
...
function assertBrowserSafeTopLevelBundle() {
  ...
  for (const forbidden of ['node:fs', 'node:path', 'node:crypto', 'node:fs/promises']) {
    if (indexBundle.includes(forbidden)) {
      throw new Error(`Top-level bundle contains Node-only import token: ${forbidden}.`);
    }
  }
}
