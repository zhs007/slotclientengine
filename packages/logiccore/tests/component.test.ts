...
describe('component mapping', () => {
  ...
  it('keeps protobuf Any component raw data without faking basicComponentData', () => {
    ...
    expect((pay?.raw as any).type_url).toBe('type.googleapis.com/sgc7pb.MoneyTriggerData');
  ...
  it('throws when a triggered component is missing in mapComponents', () => {
    ...
    delete (message.gmi.replyPlay.results[0].clientData.curGameModParam.mapComponents as any)[
      'bg-spin'
    ];
  ...
  it('throws when component basic data has invalid or out-of-range indexes', () => {
    ...
    (
      invalidUsedScenes.gmi.replyPlay.results[0].clientData.curGameModParam.mapComponents[
        'bg-spin'
      ].basicComponentData as any
    ).usedScenes = [99];
    expect(() =>
      createGameLogic(invalidUsedScenes).getStep(0).getComponentScenes('bg-spin')
    ).toThrow(LogicParseError);

    const invalidUsedResults = cloneFixture(basicMessage);
    (
      invalidUsedResults.gmi.replyPlay.results[0].clientData.curGameModParam.mapComponents[
        'bg-trigger-x5'
      ].basicComponentData as any
    ).usedResults = [99];
    expect(() =>
      createGameLogic(invalidUsedResults).getStep(0).getComponentResults('bg-trigger-x5')
    ).toThrow(LogicParseError);

    const nonArrayIndexes = cloneFixture(basicMessage);
    (
      nonArrayIndexes.gmi.replyPlay.results[0].clientData.curGameModParam.mapComponents[
        'bg-spin'
      ].basicComponentData as any
    ).usedScenes = 'bad';
    expect(() =>
