...
describe('loadGameConfigFromJsonFile', () => {
  ...
  it('fails clearly when the file does not exist', async () => {
    ...
    await expect(loadGameConfigFromJsonFile(missingPath)).rejects.toThrow(LogicParseError);
    await expect(loadGameConfigFromJsonFile(missingPath)).rejects.toThrow(missingPath);
  });
  ...
  it('fails clearly when the path is not a file', async () => {
    ...
    await expect(loadGameConfigFromJsonFile(nestedDir)).rejects.toThrow(LogicParseError);
    await expect(loadGameConfigFromJsonFile(nestedDir)).rejects.toThrow(nestedDir);
  });

  it('fails clearly for invalid JSON syntax', async () => {
    const filePath = writeTempFile('broken.json', '{');

    await expect(loadGameConfigFromJsonFile(filePath)).rejects.toThrow(LogicParseError);
    await expect(loadGameConfigFromJsonFile(filePath)).rejects.toThrow(filePath);
  });

  it('fails clearly for valid JSON that does not match the game config contract', async () => {
    const filePath = writeTempFile('invalid-contract.json', JSON.stringify({ paytable: {} }));

    await expect(loadGameConfigFromJsonFile(filePath)).rejects.toThrow(LogicParseError);
    await expect(loadGameConfigFromJsonFile(filePath)).rejects.toThrow(filePath);
    await expect(loadGameConfigFromJsonFile(filePath)).rejects.toThrow('Invalid game config');
  });
});
