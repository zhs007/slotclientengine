...
describe('parseScene', () => {
  ...
  it('throws on invalid scene structure instead of returning an empty scene', () => {
    ...
    expect(() => parseScene({ values: ['bad-column'] })).toThrow(LogicParseError);
    expect(() => parseScene({ values: [{ values: 'bad' }] })).toThrow(LogicParseError);
    expect(() => parseScene({ values: [{ values: [1.5] }] })).toThrow(LogicParseError);
  });
});
