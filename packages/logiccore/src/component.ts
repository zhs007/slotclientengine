...
export function hasTriggeredComponent(step: ParsedGameLogicStepData, name: string): boolean {
...
export function buildLogicComponent(
  ...
  if (!hasOwn(step.mapComponents, name)) {
    throw new LogicParseError(`step[${step.index}] component "${name}" is missing in mapComponents.`);
  }

  const raw = assertRecord(step.mapComponents[name], `step[${step.index}].mapComponents.${name}`);
  ...
  assertIndexesInRange(usedSceneIndexes, step.scenes.length, 'scene', step.index, name);
  assertIndexesInRange(usedResultIndexes, step.results.length, 'result', step.index, name);
...
export function getComponentScenesForStep(
  ...
  return freezeArray(component.usedSceneIndexes.map((index) => step.scenes[index]));
}
...
export function getComponentResultsForStep(
  ...
  return freezeArray(component.usedResultIndexes.map((index) => step.results[index]));
}
