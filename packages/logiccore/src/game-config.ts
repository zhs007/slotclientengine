...
export class LogicGameConfigModel implements LogicGameConfig {
  ...
  constructor(data: ParsedGameConfigData) {
    ...
    this.#reelsByName = Object.freeze(
      Object.fromEntries(
        Object.entries(data.reels).map(([name, reels]) => [name, new LogicReelsModel(name, reels)])
      )
    );
    Object.freeze(this);
  }
  ...
  getStopYCoordinates(options: ReelStopYOptions): readonly number[] {
    ...
    const reelsName = assertNonEmptyString(optionsRecord.reelsName, 'reelStopYOptions.reelsName');
    const sceneName = assertNonEmptyString(optionsRecord.sceneName, 'reelStopYOptions.sceneName');
    const scene = parseSceneMatrix(optionsRecord.scene, `reelStopYOptions.scene "${sceneName}"`);
    ...
    return freezeArray(
      scene.map((visibleSymbols, x) => {
        if (visibleSymbols.length === 0) {
          throw new LogicParseError(`Scene "${sceneName}" column ${x} must contain visible symbols.`);
        }
  ...
  getSpinStartYCoordinates(options: ReelSpinStartYsOptions): readonly number[] {
    ...
    const finalYs = assertArray(optionsRecord.finalYs, 'reelSpinStartYsOptions.finalYs');
    const reels = this.getReels(reelsName);
    ...
    return freezeArray(
      finalYs.map((finalY, x) =>
        reels.calculateSpinStartY({
          x,
          finalY: assertFiniteNumberForSpinStart(finalY, `reelSpinStartYsOptions.finalYs[${x}]`),
...
function parsePaytable(value: unknown): Readonly<Record<string, GameConfigPaytableEntry>> {
  ...
  for (const [key, rawEntry] of Object.entries(paytableRecord)) {
    ...
    const code = assertSafeNonNegativeInteger(entryRecord.code, `${entryPath}.code`);

    if (code !== codeFromKey) {
      throw new LogicParseError(`${entryPath}.code ${code} must match paytable key ${key}.`);
    }

    const symbol = assertNonEmptyString(entryRecord.symbol, `${entryPath}.symbol`);
    if (symbols.has(symbol)) {
      throw new LogicParseError(`gameConfig.paytable contains duplicate symbol "${symbol}".`);
    }
    symbols.add(symbol);
  ...
  if (Object.keys(entries).length === 0) {
    throw new LogicParseError('gameConfig.paytable must contain at least one entry.');
  }
...
function parseSymbolCodes(
  ...
  for (const [symbol, rawCode] of Object.entries(symbolCodesRecord)) {
    if (symbol.length === 0) {
      throw new LogicParseError('gameConfig.symbolCodes cannot contain an empty symbol key.');
    }

    const code = assertSafeNonNegativeInteger(rawCode, `gameConfig.symbolCodes["${symbol}"]`);
...
function parseReels(
  ...
  for (const [name, rawReels] of Object.entries(reelsRecord)) {
    if (name.length === 0) {
      throw new LogicParseError('gameConfig.reels cannot contain an empty reels name.');
    }

    const reelSetPath = `gameConfig.reels["${name}"]`;
    const reelSet = assertArray(rawReels, reelSetPath);
    if (reelSet.length === 0) {
      throw new LogicParseError(`${reelSetPath} must contain at least one reel.`);
    }

    reelsByName[name] = freezeArray(
      reelSet.map((rawReel, x) => parseReel(rawReel, `${reelSetPath}[${x}]`, paytable))
    );
  }

  if (Object.keys(reelsByName).length === 0) {
    throw new LogicParseError('gameConfig.reels must contain at least one reels set.');
  }
...
function parseReel(
  ...
  const symbols = assertArray(value, path).map((rawSymbol, y) => {
    ...
    if (paytable[String(symbolCode)] === undefined) {
      throw new LogicParseError(`${path}[${y}] references unknown paytable code ${symbolCode}.`);
    }
...
function parsePays(value: unknown, path: string): readonly number[] {
  const pays = assertArray(value, path).map((pay, index) => assertInteger(pay, `${path}[${index}]`));
...
function parseCodeKey(key: string, path: string): number {
  if (!/^(0|[1-9]\d*)$/.test(key)) {
    throw new LogicParseError(`${path} must be a stringified non-negative safe integer.`);
  }
