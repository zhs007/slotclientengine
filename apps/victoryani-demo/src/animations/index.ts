...
import {
  ...
  getNumberParam,
  getStringParam,
} from "../runtime/animation-context.js";
import {
  createNoiseCanvas,
...
export function registerBuiltinAnimations(registry: AnimationRegistry) {
  ...
  registry.register(
    "plexus",
    ({ container, target, duration, params, registerCleanup }) => {
      const graphics = new Graphics();
      const count = Math.max(
        2,
        Math.floor(getNumberParam(params, "count", 20)),
      );
            ...
            for (
              ...
              for (
                ...
                graphics.lineStyle(
                  1,
                  0x7bc8ff,
                  1 - Math.sqrt(squaredDistance) / distance,
                );
  ...
  registry.register(
    ...
    ({ container, target, duration, params, registerCleanup }) => {
      ...
      const count = Math.max(
        8,
        Math.floor(getNumberParam(params, "count", 40)),
      );
  ...
  registry.register(
    ...
    ({ container, target, duration, params, registerCleanup }) => {
      ...
      const count = Math.max(
        1,
        Math.floor(getNumberParam(params, "count", 30)),
      );
    ...
    ({ container, target, duration, params, registerCleanup }) => {
      ...
      for (let index = 0; index < count; index += 1) {
        ...
        if (texture === Texture.WHITE) {
          ...
          const ratio =
            size / Math.max(texture.width || size, texture.height || size);
  ...
  registry.register("sequenceScale", ({ target, params }) => {
    ...
    timeline.fromTo(
      ...
      {
        alpha: getNumberParam(params, "s1_endAlpha", 1),
        duration: firstDuration,
        ease: "power2.out",
      },
      0,
      ...
      timeline.add(() => {
        for (let row = 0; row < rows; row += 1) {
          for (let col = 0; col < cols; col += 1) {
            ...
            piece.endFill();
            piece.x =
              target.x + (col - cols / 2 + 0.5) * pieceWidth * target.scale.x;
            piece.y =
              target.y + (row - rows / 2 + 0.5) * pieceHeight * target.scale.y;
