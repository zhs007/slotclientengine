...
export type SceneCameraStateOptions = Partial<
  Pick<
    SceneCameraState,
    "isActive" | "x" | "y" | "scale" | "minScale" | "maxScale"
  >
>;
...
export function clampCameraScale(
  scale: number,
  minScale = DEFAULT_CAMERA_MIN_SCALE,
  maxScale = DEFAULT_CAMERA_MAX_SCALE,
) {
  const safeMin = Number.isFinite(minScale)
    ? minScale
    : DEFAULT_CAMERA_MIN_SCALE;
  const safeMax = Number.isFinite(maxScale)
    ? Math.max(safeMin, maxScale)
...
export function createSceneCameraState(
  options: SceneCameraStateOptions = {},
): SceneCameraState {
  ...
  return {
    ...
    x: options.x ?? 0,
    y: options.y ?? 0,
    scale: clampCameraScale(
      options.scale ?? DEFAULT_CAMERA_SCALE,
      minScale,
      maxScale,
    ),
    minScale,
...
export function setSceneCameraActive(
...
export function startSceneCameraDrag(
  state: SceneCameraState,
...
export function updateSceneCameraDrag(
  ...
  const safeLayoutScale =
    Number.isFinite(layoutScale) && layoutScale > 0 ? layoutScale : 1;

  return {
    ...state,
    x:
      state.dragStartCameraX +
      (pointer.clientX - state.dragStartClientX) / safeLayoutScale,
    y:
      state.dragStartCameraY +
      (pointer.clientY - state.dragStartClientY) / safeLayoutScale,
  };
}
...
export function zoomSceneCamera(
  ...
  const nextScale = clampCameraScale(
    state.scale * Math.exp(-deltaY * CAMERA_ZOOM_SENSITIVITY),
    state.minScale,
    state.maxScale,
  );
