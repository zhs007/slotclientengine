...
export function parseGame001Env(
  ...
  assertLiveWebSocketUrl(serverUrl);
...
function assertLiveWebSocketUrl(value: string): void {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
    throw new Error(
      "VITE_GAME001_SERVER_URL must be a valid ws:// or wss:// URL.",
    );
  }
  if (url.protocol !== "ws:" && url.protocol !== "wss:") {
    throw new Error(
      "VITE_GAME001_SERVER_URL must use ws:// or wss:// live mode.",
    );
  }
}
