...
项目外层格式为 strict v3：保存 layout hash、现有 scene/choreography flow 与
`slot-operation-authoring-project@1`。v2 导入必须先加载匹配 ZIP，并显式接受升级草稿；
在全部 edge `review: complete` 且 strict finalizer 闭合前，预览与导出禁用。

相邻 snapshot 会生成 spin、settled replacement 和 value update operation evidence。
...
闭合后才能逐 edge 接受。
launch v3 只通过一次性 MessageChannel 发送 ZIP、flow 和 finalized plan；预览 runtime
在创建 Pixi/runtime 资源前核对 plan initial 与每个 authored output checkpoint。Replay
...
外层 v3 中的 presentation flow 继续使用 strict `scene-other-scene-flow` v2。首边 Spin 是一个合并编排，固定由“Spin 前 → Spin 中 → 停止”三个节点组成；停止和普通 Sequence 都以 exact `normal` 结尾，中间项只允许由真实动画 completion 推进的 once state，不使用 hold timer。
