...
function createAssetPaths() {
  return Object.fromEntries(
    cabinAnimationData.attachmentNames.map((name) => [
      name,
      `./assets/${name}.png`,
    ]),
  );
...
function getFirstVisibleFrame(script: string) {
  const payload = JSON.parse(script) as {
    frames: Array<[number, number, number, number, number, number, 0 | 1]>;
  };
...
describe("mirror mapping", () => {
  it("keeps known UI branch pairs mirrored around the export stage anchor", () => {
    ...
    for (const [leftId, rightId] of MIRROR_LAYER_PAIRS) {
      ...
      expect(leftFrame[0] + rightFrame[0]).toBeCloseTo(
        EXPORT_STAGE.anchorX * 2,
        1,
      );
