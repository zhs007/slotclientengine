...
describe("sampleAnimationPose", () => {
  it("samples slot attachment and color timelines", () => {
    const earlyPose = sampleAnimationPose(
      cabinAnimationData,
      "cabin",
      0.2,
      true,
    );
    const latePose = sampleAnimationPose(
      cabinAnimationData,
      "cabin",
      1.5,
      true,
    ...
    expect(
      sampleAnimationPose(cabinAnimationData, "cabin_s", 0, true).slots.ui31
        .color,
    ).toBe("ffffff9f");
  });
  ...
  it("applies translated world transforms through the bone hierarchy", () => {
    ...
    expect(worldBones.ping2_01.x).not.toBe(
      cabinAnimationData.bones.find((bone) => bone.name === "ping2_01")?.x,
    );
