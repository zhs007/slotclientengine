...
import {
  buildDebugTree,
  createDebugNodeIndex,
  getBoneDebugNodeId,
  getSlotDebugNodeId,
} from "../../src/runtime/debug-tree.js";
...
describe("buildDebugTree", () => {
  it("creates a stable tree that covers every bone and slot", () => {
    ...
    expect(tree.length).toBe(
      cabinAnimationData.bones.filter((bone) => bone.parentName === null)
        .length,
    );
    expect(index.size).toBe(
      cabinAnimationData.bones.length + cabinAnimationData.slots.length,
    );
  });
