...
describe("game002 Leo platform label mapping", () => {
  it("maps only explicit known keys and falls back field by field", () => {
    const labels = createGame002LeoUiLabels({
      spin: "Girar",
      common_label_Balance: "Saldo",
      common_label_win: "Premio",
      common_label_Totalbet: "Apuesta total",
      ready: "",
      unknownAction: "must not spread",
    });
    expect(labels.spin).toBe("Girar");
    expect(labels.balance).toBe("Saldo");
    expect(labels.win).toBe("Premio");
    expect(labels.totalBet).toBe("Apuesta total");
  ...
  it("keeps the Task 121 aliases as controlled compatibility fallbacks", () => {
    expect(
      createGame002LeoUiLabels({ balance: "Legacy balance" }).balance,
    ).toBe("Legacy balance");
  });
});
