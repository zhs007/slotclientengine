...
`skin=2` 当前可贴图 symbol 是 `WL`、`H1`、`H2`、`L1`、`L2`、`L3`、`L4`、`WM`、`CN`、`CM`、`CO`、`AF`。`skin=3` 当前可贴图 symbol 是 `WL`、`H1`、`H2`、`L1`、`L2`、`L3`、`L4`、`CN`、`CO`。`BN` 是 empty symbol，不要求图片。
...
`release:check` 会确认 dist 同时包含 `skin=2` 和 `skin=3` 的背景、普通 symbol、`spinBlur` 和 `disabled` 资源。因为两套 symbol 有同名文件，检查会按构建产物图片尺寸区分 `200 x 200` 的第二套和 `180 x 180` 的第三套。
...
- 缺少 `skin`、`skin` 重复、`skin` 为空，或 `skin` 不是 `2` / `3`。
...
- `skin=2` 的 `bgfull.jpg` 或 `skin=3` 的 `bg.jpg` 尺寸不是 `2000 x 2000`。
