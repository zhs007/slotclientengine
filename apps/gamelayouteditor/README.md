...
多个通用主状态，每个状态按 active variant 绑定背景、stateful Spine node 稳定状态、零个或
一个 symbols package，以及零个或一个 popup。导入不会自动绑定。
...
- 多个主状态在同一 variant 选择同一个 Spine logical resource 时复用一个背景 node；编辑器把各模式选择的稳定 animation 映射为 state machine state。BG→FG、FG→BG 等有向 transition 仍须在背景 Inspector 显式选择真实 once animation，预览切换时完整播放后再进入目标 loop。选择不同 resource 才分叉背景 node。
...
binding 会在 mutation 前生成并验证目标 scene，在 transition 完成的提交边界替换。目标无 symbols
...
相同 binding 不传 target input，目标无 symbols 也不传。完成后再调用
