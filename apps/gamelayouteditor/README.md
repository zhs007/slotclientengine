...
Popup Object ZIP 从 Assets 上传入口导入后只加入对象库，不会自动绑定。项目 Tab 的“Tap info Popup Object”可显式选择一个对象或“未配置”；删除当前已绑定对象会失败，需先清空绑定。只有被选择对象进入导出闭包，重新导入导出 ZIP 会恢复该选择。选中对象会以 Popup order 显示在默认黑色 Splash 和 authored Splash 的 viewport center；普通 Spine Popup 还需在自身 manifest 配置 exact Tap info 父节点，才会额外显示自己的 attached instance。
