...
- `转场`：维护独立的有向 mode edge。Spine 分支选择已有 Spine resource、真实 once animation、唯一 event 与 per-variant art-space placement；video 分支选择已有 MP4、固定 `contain` 和正数 `fadeOutSeconds`。编辑器只自动准备当前 stable source 到预览 selector 所选 target 的一条直接边，并通过统一的“切换到该状态”动作启动。
...
- 带声音 video 仍使用两阶段安全边界，但 editor 不把底层 prepare/cancel 暴露为产品按钮：目标选择后自动调用 `prepareGameModeTransition()`，只有 MP4 media readiness、真实 metadata、Pixi presentation 和完整目标 scene 都准备完成后才启用“切换到该状态”。该按钮的真实 click listener 在任何 `await` 前直接调用 `requestGameMode()`，使 runtime 同步调用 audible `video.play()`；`play()` 拒绝时清理 prepared owner、保持来源 stable scene 并显示原始错误，不静音重试、瞬切或记录异步 autoplay intent。
...
layout ZIP 中的 owned MP4 是完整、精确的 payload bytes。Production 可以在游戏初始化或 phased
loading 阶段尽早下载这些静态 bytes；但“资源已下载”不等于转场已经 prepared。一次完整 prepare
还包括创建 audible inline `HTMLVideoElement`、设置 `preload=auto`、等待浏览器 media readiness、
用真实 metadata 校验 duration/尺寸/fade 边界、创建 Pixi video texture/presentation，以及准备目标
mode 的 reel、catalog 与完整 scene owner。浏览器自行决定 HTTP cache、media buffer、内存与 GPU
资源的保留和淘汰，不能承诺整段 MP4 永久驻留或仅凭 URL/bytes 已缓存就能立即播放。

业务已知道下一个可能目标、并已取得构造目标 scene 所需输入时，应尽早调用
`prepareGameModeTransition(modeId, options)`。玩家最终确认的真实 click/pointer listener 必须直接、
同步调用 `requestGameMode(modeId, sameOptions)`；不能在 prepare Promise、网络请求或
`canplaythrough` 回调中自动播放有声视频。一次“进入游戏”点击或一次成功播放不是永久、全页面的
audible media 授权。连续媒体优先复用已经成功启动过的同一个 media element 是 WebKit 上的可靠性
优化，但每一次 `play()` Promise 的实际 resolve/reject 仍是唯一事实。当前 runtime 保持
per-transition element；多目标 media-only pool、内存预算和淘汰策略不属于 editor workflow。

Production 必须监听 `visibilitychange`、`pagehide`、`pageshow`，以及 media 的
`pause/error/stalled/playing`。iOS 从后台恢复后，应先读取 visibility、`paused/ended/error/readyState`
和 runtime 的 before/after-switch 边界，再尝试恢复同一个 element；恢复 `play()` 被拒绝时显示明确的
“点按继续转场”入口。before-switch 保持来源 scene，after-switch 保持已经提交的目标 scene；不得静音、
跳过动画、从头重播或重复提交切场边界。spin click 后经过网络 Promise 才发生的转场已经离开原始
trusted-click 同步调用栈，不能因“本轮由点击开始”就假定有声播放获准。
