...
以及零个或一个 popup。模式之间的 Spine 转场在独立有向边中配置；导入不会自动绑定。
...
新建项目 dialog / 导入
  ...
  -> 转场 Tab 显式创建有向边，选择 Spine once animation、唯一 switch event 与 placement
...
- `转场`：维护独立的有向 mode edge；选择已有 Spine resource、真实 once animation、该 animation 中恰好一次的 event，以及每个 active variant 的 art-space placement。
...
- 不同主状态的背景 node 始终独立。选择同一个 Spine logical resource 只复用 payload bytes，不复用 player、placement 或 node。`BG -> FG` 与 `FG -> BG` 必须在“转场”Tab 分别创建，不能猜名称、自动补反向边或用 once completion 代替 switch event。
...
Promise 在 switch event 已原子提交完整目标场景、且 overlay once 播放完成后 resolve。期间可通过
`getGameModeSnapshot()` 区分 `before-switch` 与 `after-switch`。完成后再调用
