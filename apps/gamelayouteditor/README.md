...
未引用资源不会阻止导出，也不会进入 production ZIP。它们在导出后继续留在当前会话，但重新导入该 ZIP 时不会恢复。只有被 node 引用的 image-string 会 vendor；Symbols package 默认仅预览，只有明确选择“随布局包导出”才会保存 binding 并 vendor 完整 package。
