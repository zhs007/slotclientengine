...
- 上传图片只创建一个 image logical resource 和对应 bytes；上传 Spine 文件组只创建一个由 skeleton、atlas 与全部 atlas pages 构成的 Spine logical resource；上传 standalone image-string ZIP 会保留 validated manifest 与完整 glyph bytes。
