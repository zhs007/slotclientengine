...
const inputSymbols = new Uint8Array(
  await readFile(resolve(outputDirectory, "game002-s3-symbols-task131.zip")),
);
...
try {
  ...
  try {
    ...
    if (
      ...
      throw new Error("Task 131 layout lost a game mode Symbols binding.");
    }
  } finally {
    reimported.destroy();
  }
  ...
  const outputPath = resolve(outputDirectory, "crave-layout-task131.zip");
...
function dependencyFiles(
  ...
  return new Map(
    keys.map((key) => {
      ...
      if (!bytes) throw new Error(`Task 131 dependency is missing ${key}.`);
...
function assertEditorProject(project: EditorProject): void {
  if (!project.symbolDependencies.has("game002-s3")) {
    throw new Error("Task 131 layout is missing game002-s3.");
  }
  for (const modeId of ["BaseGame", "FreeGame"]) {
    ...
    if (mode?.symbols?.packageId !== "game002-s3") {
      throw new Error(
        `Task 131 layout mode ${modeId} is not bound to game002-s3.`,
      );
    }
  }
}
...
function decodeImageSize(bytes: Uint8Array): { width: number; height: number } {
  ...
  throw new Error("Task 131 encountered an unsupported image payload.");
}
