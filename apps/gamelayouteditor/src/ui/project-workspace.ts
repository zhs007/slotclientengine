...
export function projectWorkspaceMarkup(
  ...
  const referenced = [...project.resources.keys()].filter(
    (id) => {
      if (getLayoutResourceReferences(project, id).length > 0) return true;
      const resource = project.resources.get(id);
      return (
        resource?.kind === "popup-object" &&
        resource.manifest.name === project.tapInfoObjectName
      );
    },
  );
