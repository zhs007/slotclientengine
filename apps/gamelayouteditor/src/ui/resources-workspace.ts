  ...
  const rows = [...project.resources.values()]
    ...
    .filter((resource) => {
      ...
      const projectBound =
        ...
        project.tapInfoObjectName === resource.manifest.name;
      ...
      if (session.resourceStatus === "unused")
        return references.length === 0 && runtimeKey === null && !projectBound;
...
function resourceRowMarkup(
  ...
  const preview =
    resource.kind === "image" && thumbnailUrl
      ...
      : `<span aria-hidden="true">${resource.kind === "spine" ? "SP" : resource.kind === "vni" ? "VNI" : resource.kind === "image-string" ? "TXT" : resource.kind === "popup-object" ? "OBJ" : resource.kind === "json" ? "JSON" : resource.kind === "video" ? "MP4" : resource.kind === "audio" ? "AU" : "IMG"}</span>`;
  return `<article class="resource-row" data-resource-row="${escapeHtml(resource.id)}">
    ...
    <div class="resource-actions">
      ...
      ${resource.kind === "popup-object" ? '<span class="hint">由项目 Tap info 绑定管理；可手动添加为 Scene 图层</span>' : `<label>程序键<input data-runtime-resource-key="${escapeHtml(resource.id)}" value="${escapeHtml(runtimeKey ?? suggestRuntimeResourceKey(resource.id))}" placeholder="例如 nearwin" /></label><button type="button" data-runtime-resource-action="${escapeHtml(resource.id)}" data-runtime-bound="${runtimeKey !== null}">${runtimeKey ? "取消强制导出" : "设为程序资源"}</button>`}
      ...
      ${resource.kind === "popup-object" ? "" : `<button type="button" class="danger" data-delete-resource="${escapeHtml(resource.id)}" ${references.length > 0 ? `title="被 ${references.map((reference) => reference.nodeId).join(", ")} 引用"` : ""}>删除</button>`}
    </div>
    ${expanded ? resourceDetailsMarkup(resource, references, runtimeKey, projectBound) : ""}
  </article>`;
}
    ...
    resource.kind === "image"
      ...
      : resource.kind === "spine"
        ...
        : resource.kind === "vni"
          ...
          : resource.kind === "popup-object"
            ? `<li>manifest: ${escapeHtml(resource.manifestPath)}</li><li>Popup Object ${escapeHtml(resource.manifest.name)} · ${resource.assetPaths.length} assets</li>`
            : resource.kind === "video"
