...
describe("symbols editor app shell", () => {
  ...
  it("imports generic resource ZIPs and reviews conflicting replacements", async () => {
    ...
    Object.defineProperty(upload, "files", {
      configurable: true,
      value: [new File([secondBytes as BlobPart], "H1.png")],
    });
    upload.dispatchEvent(new Event("change", { bubbles: true }));
    await vi.waitFor(() =>
  ...
  it("imports multiple Spine skeletons that share one atlas and texture", async () => {
    ...
    const files = names.map(
      (name) => new File([readCraveFixture(name) as BlobPart], name),
    );
    Object.defineProperty(upload, "files", {
      configurable: true,
      value: files,
  ...
  it("still rejects a Spine import when multiple atlases make the closure ambiguous", async () => {
    ...
    Object.defineProperty(upload, "files", {
      configurable: true,
      value: [
        new File([skeleton as BlobPart], "CN_1.json"),
        new File([atlas as BlobPart], "Symbol.atlas"),
        new File([atlas as BlobPart], "Other.atlas"),
        new File([texture as BlobPart], "Symbol.png"),
      ],
    });
    upload.dispatchEvent(new Event("change", { bubbles: true }));
