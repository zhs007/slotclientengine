...
锻铁贴图。宝箱、分段圆柱、城堡凳、木桶、边墙/贴墙柱和壁挂火把按 img2threejs 的参考准入、严格
雕塑规格、无贴图结构门禁和最终贴图多角度门禁完成重建。运行时贴图统一使用 WebP；参考图、细节
裁剪和验收截图只保留在临时工作目录，不进入 Git。
...
开发时可用 `?prop=chest|column|bench|barrel|wall|torch` 查看对应道具的无贴图结构门禁；追加
`&mode=final` 查看最终贴图，追加 `&view=side` 检查侧视体积。
