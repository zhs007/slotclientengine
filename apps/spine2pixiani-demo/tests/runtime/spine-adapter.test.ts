...
import {
  ...
  asset12AnimationNames,
} from "../../src/data/asset-12-animation-data.js";
...
describe("adaptSpineData", () => {
  it("builds the internal model with bones, slots, attachments, and animations", () => {
    ...
    expect(Object.keys(cabinAnimationData.animations)).toEqual([
      "cabin",
      "cabin_s",
    ]);
    expect(
      cabinAnimationData.animations.cabin.bones.ping2_01.translate.length,
    ).toBeGreaterThan(0);
    expect(
      cabinAnimationData.animations.cabin.slots.ping1_01.color.length,
    ).toBeGreaterThan(0);
    expect(
      cabinAnimationData.slots.find((slot) => slot.name === "ping_g")
        ?.blendMode,
    ).toBe("additive");
  ...
  it("adapts asset 12 animations with shear and drawOrder timelines", () => {
    expect(asset12AnimationNames).toEqual([
      "bonus1",
      "bonus2",
      "bonus3",
      "bonus4",
