...
import {
  ...
  panViewport,
  zoomViewportAtPoint,
} from "../../src/runtime/viewport-controller.js";
...
describe("viewport-controller", () => {
  ...
  it("zooms around an anchor point and clamps zoom range", () => {
    ...
    expect((anchor.x - next.panX) / next.zoom).toBeCloseTo(
      (anchor.x - state.panX) / state.zoom,
    );
    expect((anchor.y - next.panY) / next.zoom).toBeCloseTo(
      (anchor.y - state.panY) / state.zoom,
    );
