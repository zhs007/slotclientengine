...
- `--delivery-dir` 生成 runtime/CDN 直接消费的 versioned delivery 扁平文件池。它从 typed dependency graph
...
- legacy optimized ZIP 把 audio `runtimeResources` 明确作为 effect role 执行既有 AAC 优化；同一 root 同时被 Event music 使用时仍由 music role 优先，优化后的 path/mediaType 必须在所有 typed reference 中原子改写。delivery 输出继续保留输入 bytes/container。
...
新输出恒为 `scene-layout-delivery` version 2。version 2 的 CDN path 必须是与声明 SHA-256 一致的单段文件名；
...
- Popup reference rewrite 只依赖 `rendercore/popup/data`，任一受支持的 v1–v9 source manifest 都先
  strict 规范化为 latest v9；v7–v9 typed audio、文字宽度区间与三种 Popup 的资源引用均结构化改写，CLI 不加载 mapped editor
...
- `assets.map.json` 的 path、SHA-256、media type 和 byte length 按优化后 bytes
  重新生成；
- root Scene Layout 旧音频目录与 v5/v6 event audio、nested Symbols 与 Popup 的 typed 音频 path 同步改成 `.m4a`，media type
