...
- `game002` 的消除级联组件名、step 映射和 symbol 特例属于 `apps/game002`；`packages/rendercore` 拥有通用 manifest-driven emphasis/win/remove、非中奖格与其 symbol 本体同步压暗及渐变恢复、可注入 remove/drop predicate，以及把既有 occurrence 与 refill occurrence 合并的 grid-cell fall。game002 的强调段固定为 `0.1s` 渐暗、`1s` 保持、`0.1s` 渐亮；恢复后必须严格按组执行 `win 完成 -> 立即 remove -> 下一组 win`。game002 必须通过函数声明 `WL` 不 remove、不 drop；rendercore 不得硬编码 wild。game002 spin 只让 `WL/CN` 保持全亮；其它实际滚动 occurrence 的格底使用随 reel slot 同速移动的 `0.82` 半透明黑层，symbol 同步使用灰阶 tint 变黑且 alpha 保持 `1`。暗层不得固定在棋盘坐标形成矩形断层，也不得退回奇偶棋盘格；rendercore 只接收通用 symbol-code resolver，不得硬编码 `WL/CN`。普通 spin 回弹力度只从独立 `assets/game002-s3/reel.manifest.json` 的 `spin.bounceStrength` 读取，当前 game002 为 `0`、完全不回弹；rendercore 通用 parser/runtime 以 `1` 表示原始力度并按非负值缩放，app 不得再硬编码第二份数值。普通 symbol 可穿过固定 WL 下落，叠放只服从 manifest `renderPriority`，当前 game002-s3 仅 WL 为 `1`、其它为 `0`。统一 fall 不走 spin/appear，并同时保留列内错峰和从左到右列延迟；normal 与 dropdown 实际 animation resource/playback 完全一致时，RenderSymbol 必须保留 player 和时间轴，不能 reset/replay 等价 Loop；不同动画、reset 或 value/tier 变化仍正常切换。fall 期间只给整个 grid-cell reel set 启用一个完整轮子矩形 mask，活动 symbol 自身不得再叠加 mask，从棋盘上方进入的 symbol 未进入轮子区不得显示；game002 当前使用短促下落、较大 overshoot 和快速 settle。CN 数字仍随 occurrence 下落；只有 refill 新增 CN 时才要求当前 step 触发 `bg-gencoins` 并使用服务端值，没有新增 CN 时该组件允许缺失且既有 CN value 必须从 dropdown occurrence 严格携带。全部 step 结束后才播放 global win-amount；win-amount 播放期间仍必须逐帧更新 reel runtime，不能冻结 CN/其它 symbol 的 normal Loop。不得在 app 复制 Pixi/Spine/cascade 算法或增加 fallback。
...
- `game002-s3` 的可展示 symbol 固定为 `WL,H1,H2,L1,L2,L3,L4,WM,CN,CM,CO,AF,BN`，全部由 manifest 显式声明 `scale: 1`；`BN` 是真实贴图且 `emptySymbols` 为空，不得作为缺图兜底。`CN` 禁止顶层 `normal/spinBlur/disabled`：其透明 reel normal、blur/disabled 只在 `valuePresentation.reelStates` 配置，实际 normal art 只来自分档 Spine；数字显示对象必须按 manifest 配置绑定到各档 skeleton 真实存在的 `Num` slot，继承 slot/bone 动画，不能作为同级 Pixi overlay。当前 game002-s3 使用 `text.type=image` 和 `prefix=./`，按完整 raw value 精确映射 `${value}.png`；`1,2,5,10,25,50,100,250,500,1000` 图片都属于 manifest 派生的精确资源闭包，缺对应值图片必须在 spin 前或展示前显式失败，不能回退字体。defaultScene 无 otherScene 和 spin 本地临时轮带中的 CN 必须从 `valuePresentation.defaultValues` 显式候选数组取随机值并把值绑定到临时轮带 occurrence，使档位 Spine/数字跟随实际 reel slot 滚动；最终停轴 CN 仍使用服务器 otherScene，不能用随机值覆盖服务器结果。`CN_1..CN_4` 仍不属于主 display set，只是当前 `CN.valuePresentation` manifest 实例精确引用的附属 Spine 资源；`Nearwin1..Nearwin3` 和 `WM_Fx` 仍不得接入。所有附属资源都禁止通过宽泛 glob 当 symbol 打包。
- `packages/rendercore` 拥有通用、manifest 驱动的 symbol value-presentation tier、官方 Spine player、font/image value display 和 prepare/show/update/clear/destroy 生命周期；`text.type` 缺省为 `font`，image 模式必须配置本地 path `prefix` 并使用完整数值图片。`apps/game002` 只拥有 `bg-gencoins` / `CN` otherScene 映射。档数、阈值、资源路径/命名和显示样式不得在 app 或 viewer 建第二份表，也不得依赖 `CN_数字` 命名。manifest 改动后必须重生成 game002 与 symbolsviewer 的精确 Vite/loading closure，当前工作区新美术不得被旧资源覆盖。game002-s3 当前各档 CN 的 normal 使用真实 `Loop`，逐格落地使用真实 `Start`；数字图片始终挂在同一 player 的 `Num` slot 下，`Start` 完成后同一 player 切回 `Loop`。
