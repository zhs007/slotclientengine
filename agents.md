...
- `assets/game003-s1/{WL,H1,H2,H3,H4,H5,CL,SC}.json`、`assets/game003-s1/Symbol.atlas` 和 `assets/game003-s1/Symbol.png` 是 `symbol-state-textures.manifest.json` 驱动的 Spine symbol 动画资源；当前 skeleton 为 Spine `4.2.43`，`packages/rendercore` 的官方 `spine-pixi-v8` runtime 必须锁在 `4.2.x`，依赖范围不能自动跨到 4.3。4.2 资源走 rendercore 封装的官方 runtime，3.8 内部 adapter 不是当前 game003 路径；未知/错配版本、atlas/skeleton/animation 错误必须显式失败，app/viewer 不直接依赖 Spine runtime。`WL/H1/H2/H3/H4/H5/CL/SC.normal` 必须使用 skeleton 中真实存在的 Spine `Idle`，`win` 必须使用真实存在的 Spine `Win`，`WL.appear` 使用真实存在的 lowercase `start`，`H1/CL/SC.appear` 使用真实存在的 `Start`。
