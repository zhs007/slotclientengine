...
- `game003` 的 `bg-bar` 传送带玩法属于 `apps/game003` app 层，组件名固定为 `bg-bar`，只支持 `normal|wild|up`，每次 spin 必须在主转轮启动同时根据本轮 `FeatureBar2Data.features` 启动，`playSpin()` 必须等待主转轮、`bg-bar` 终点 win、`bg-wins`、矿车互动和中奖金额主要播放完成；中奖金额进入 `awaiting-dismiss` 后不再阻塞 `playSpin()`，下一次 spin 开始时自动关闭上一轮金额展示。金额动画点击只做 `requestAdvance()` 跳档：不到 bigwin 直接跳最终金额，到 bigwin 以上每点一次跳到下一档，最后点击也不能隐藏，必须等下一次 spin 清屏。`normal` 是 `bg-bar-symbol-state-textures.manifest.json` 显式声明的透明 symbol，不新增透明 PNG，`wild.png` 为 `172 x 158`，`up.png` 为 `172 x 130`，slot 坐标必须来自 YAML 的显式 `slotRectsInConveyor`，不要在 app 内用 conveyor 宽高除 5 或在共享包里硬编码 `bg-bar` / `wild` / `up` / conveyor 语义。
...
- `assets/game003-s1/{WL,H1,H2,H3,H4,H5}.json`、`assets/game003-s1/Symbol.atlas` 和 `assets/game003-s1/Symbol.png` 是 `symbol-state-textures.manifest.json` 驱动的 Spine symbol 动画资源；新增或调整 Spine symbol 动画时，必须同步 manifest、YAML Spine glob、loading 资源、生成物、symbolsviewer 预览和 game runtime resolver。Spine animation name 区分大小写，manifest 必须写资源中真实存在的 exact name。
...
- `game003` 的金额显示当前按服务器整数 `100 -> $1.00` 转换；framework HUD 和 Pixi 中奖金额动画必须复用同一套 formatter，不要在动画里直接渲染服务器整数；Pixi 中奖金额动画不以点击关闭作为 `playSpin()` resolve 的必要条件，玩家点击只能加速/跳档，下一次 spin 开始时自动关闭上一轮展示。
