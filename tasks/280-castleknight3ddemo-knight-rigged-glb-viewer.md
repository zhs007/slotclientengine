...
- 第七轮用户截图显示 walk 的零横向腿平面约束导致膝盖内扣、鞋向中线拥挤和交叉感；替换为稳定的自然宽度轨道：
  膝盖相对髋部略向外，脚踝继续外展，左右鞋保持平行朝前且各走各自轨道。validator 删除零偏差门禁，改为膝/踝
  外展范围、轨道稳定性及左右鞋几何中心最小 14cm 间距门禁，并从与截图一致的正面渲染 walk 全部 9 个相位。
- 第八轮用户截图进一步指出大腿仍然向外斜。walk 改为髋部到膝盖保持竖直，只让小腿向自然宽度的脚踝轨道轻微
  外展；validator 将膝盖相对髋部的横向偏差限制为最大 5mm，同时保留脚踝外展、鞋尖朝前、鞋中心间距和九相位
  正面渲染门禁。
