...
| symbolseditor     | 通过   | 通过 | 通过      | 6/6（2 files）      | 通过  |
...
- symbolseditor：statements `30.60%`，branches `22.40%`；model 为 `77.66%`，ZIP IO 为 `90.47%`，Pixi/UI 留给浏览器验收；
...
### Incomplete draft 交互修正

浏览器验收继续发现新建项目会立即把缺失资源闭包显示为全局错误。该行为与任务合同“草稿可不完整、导出必须严格”不符，现已拆分为：

- project diagnostics 只报告 game config、manifest 和字段结构错误；
- 新建项目不再因尚未上传图片显示红色全局错误；
- 每个 included symbol 单独显示“资源完整”或精确 missing 列表；
- 已完整 symbol 会组成临时精确子闭包并先行预览，不受其它未完整 symbol 阻断；
- 导出按钮在整个 display set 闭包完整前禁用；
- UI 明确提示 game config symbol 默认全选，辅助 symbol 需取消勾选；
- 新增 incomplete draft / partial preview snapshot 测试，symbolseditor 当前为 6/6 通过。
