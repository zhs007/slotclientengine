...
- 浏览器视觉与交互验收：待用户执行。
- 工作区保持未提交状态，未新建分支、commit 或 push。
...
执行开始时只有任务计划 `tasks/129-gamelayouteditor-preview-coordinate-refresh.md` 为未跟踪文件；该文件已保留。
...
- 旧 manifest 缺少 `coordinateOrigin` 或 reel placement `scale` 时分别按 `top-left`、`1` 读取；Editor 再导出时写入 canonical 显式值。
...
- `orientation-focus` 的 landscape/portrait 分别提供 main reel 整体 scale；单背景 UI 不开放该字段。
- RenderCore resolver 使用缩放后 reel bounds 做 art/focus 校验，package runtime 对 reel presentation 根节点统一设置 scale，不改子 symbol scale。

### 红框与刷新分级

- Layout preview 新增独立、非交互的红色 selection overlay；普通图层选择、variant/resize、geometry change 和 Spine 每帧 bounds 变化都会更新。
...
- legacy symbol overlay 改为父容器 position/scale；相同 sampled scene 的几何更新只调整父容器，不销毁或重建 symbol 实例，也不重新随机。
...
coverage · statements 87.09% / branches 80.01% / functions 90.25% / lines 87.53%
...
coverage · statements 81.64% / branches 73.72% / functions 85.76% / lines 83.31%
...
依赖首次不可用时按仓库约定执行了 `CI=true pnpm install --frozen-lockfile`；没有修改 `package.json` 或 `pnpm-lock.yaml`。

## 待用户浏览器验收

1. 创建双背景项目，导入 image/Spine 普通图层与 Symbols，导出 ZIP 后重新导入；确认大纲与资源名可读，ZIP payload 仍是 hash。
2. 选中不同普通图层并切横竖屏、缩放 preview；确认红框跟随实际边界，黄/绿 guide 不变，隐藏图层不画假框。
3. 连续只改 node/reel x、y、scale；确认 symbols 排列、当前 mode 和 Spine 动画不重置。
4. 分别修改 landscape/portrait main reel scale；确认整个图标区等比缩放，单个 symbol 内部 scale 未被改写。
5. 往返切换左上角/中心坐标；确认背景、普通图层、Spine transition 与 main reel 视觉位置不跳。
6. 导入缺少新字段的旧包；确认按左上角、scale 1 渲染，并可重新导出。
