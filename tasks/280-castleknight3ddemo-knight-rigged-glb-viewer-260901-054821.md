...
`idle`、`attack`、`victory` 三段骨骼动画。生产 GLB 的三张纹理已转换为带完整 mipmap 的 2048×2048
UASTC KTX2；最终文件为 `7,826,324` bytes，比原始 `29,473,708` bytes 减少 `73.45%`。
...
- `apps/castleknight3ddemo/public/models/castle-knight-rigged-ktx2.glb`
  ...
  - `attack` 1.13 秒、`idle` 2.03 秒、`victory` 1.53 秒；每段 60 个 transform channels。
  ...
  - SHA-256：`490dc2652ba96ed84fc35bab3772882118bf9252a33b75eb6ff1d98bcef79264`。
...
- action 集严格等于 `attack / idle / victory`，均为非零时长。
- final 恰有 1 个 skin、20 joints、3 个同名 clips；KTX2 magic、尺寸、mipmap、extension 和最终大小全部符合合同。
- rest pose 与每个 clip 的 0/25/50/75/100% 关键姿势共 16 张 Blender 渲染均无左右错绑、悬空部件或异常拉伸。

## 自动验收

```text
Blender validate_knight_glb.py --intermediate ... --final <仓库 final GLB> --output-dir /private/tmp/task-280-independent-validation
PASS：结构、刚性权重、3 clips、3 KTX2、逐张 ktx validate、16 张姿势渲染
...
1. 默认 KTX2 骑士正确显示 base color、normal、metallic/roughness 材质；统计为 1 mesh、24,702 triangles、
   20 bones、1 skin、3 animations，默认循环播放 `idle`。
2. `attack`、`victory` 切换、单次停止、播放/暂停、重播、时间轴 scrub、loop toggle 与 skeleton toggle 均生效；
   中间姿势可见明确挥剑和举剑动作，骨架跟随模型。
...
- 模型是分件硬表面资产，极端大幅度动作仍可能出现原始甲片之间的几何穿插；本次三个动作的关键姿势和浏览器播放
  范围已控制在可接受幅度。
