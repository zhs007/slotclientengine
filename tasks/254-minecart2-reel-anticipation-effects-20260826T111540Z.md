...
- 2026-08-27 视觉跟进：轮到某轴落停时，nearwin 先做 X `1.025→1.05→1.025→1.05`、Y `1.05→1.1→1.05→1.1` 双回弹；随后只 settle 当前轴，等它物理停稳后再推进下一轴，玩法 post-land Promise 继续独立并发。
...
- wave 间隔：240ms
- 最短可见时长：180ms
- 回弹：120ms expand、100ms contract、140ms rebound；Y 增量严格为 X 的两倍
...
配置由 `apps/minecart2/config/game-runtime.manifest.json` v4 单一持有，app parser 严格校验强期待必须更慢、更久且视觉更强，并校验回弹 peak/base 与 X/Y 增量关系。
