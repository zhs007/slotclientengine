...
## 12. Game Layout Editor BG legacy texture / art size 补充（2026-07-22）

用户用 `assets/game002-s3/BG.json`、`BG.atlas` 和全部 `BG*.png` 验收时发现两个导入阻断。真实检查确认这些历史 `.png` 文件实际是 WebP 字节；atlas page 仍以 `BG.png`、`BG_2.png` 等逻辑名引用。修复后导入边界保留 atlas page 文本，按真实 bytes 将物理 filename key 规范化为 `BG.webp`、`BG_2.webp`，并通过 Spine `textures` map 建立精确 page -> key 映射。这样 legacy 素材可导入，导出的 payload 扩展名/media type/内容仍严格一致，没有增加 MIME fallback。

第二个错误来自新项目 art size 为 `0 × 0`，而 Spine export bounds `3744.3176 × 2371.955` 和 atlas page `2000 × 2000` 都不能替代完整美术坐标系合同。原始 parser 英文错误已改为指向 Resource Picker 的中文 incomplete 提示。Resource Picker 现在只在“Spine 设为背景且当前 variant art size 未完成”时显示必填 width/height；game002-s3 应明确输入 `2000 × 2000`。确认时 art size 与背景绑定在同一 transaction 中提交，初始 placement 得到 `(1000, 1000, 1)`；非法或缺失尺寸显式失败且不产生半提交。

补充自动化覆盖 legacy WebP-in-PNG page 映射、atlas page 不改写、Picker art size 初值和完整 UI 绑定流程。`gamelayouteditor` 最新 scoped test 为 `19 files / 145 tests passed`。最终真实浏览器验收仍由用户执行：导入全部 BG closure，选择 `BG` animation，填写 `2000 × 2000`，确认无 extension/artSize 错误且背景居中显示。

补充门禁首次复跑时，`editor-store` 的旧断言仍要求原始英文 `positive`，与本次有意改成中文 actionable diagnostic 的结果不一致；断言改为验证“有限正数”语义后完整复跑通过，没有放宽 parser 或跳过行为测试。
...
## 13. Symbols dependency 闭包与 macOS ZIP 导入补充（2026-07-22）

用户将 Symbols ZIP 导入 Game Layout Editor 后，编辑器能够识别根 `symbols.package.json`，但预览启动时报 `Scene layout package file is missing: gameconfig.json.`。根因不是 Symbols ZIP 缺文件，而是 `refreshPreview()` 只收集 scene-layout manifest 直接引用的 Symbols 根 manifest，没有把所绑定 Symbols dependency 的完整 filename-key 闭包传给 preview。现改为按实际 mode/variant binding 展开 Symbols、Popup 与 editor-owned image-string dependency；引用不存在的 dependency 会显式失败，不能靠猜测或 fallback 补文件。回归测试确认预览闭包同时包含 `symbols.package.json`、`gameconfig.json`、symbol manifest 与贴图。

用户进一步明确 Finder ZIP 的 `__MACOSX` 检查过严。共享 `editorresource` 现统一提供 ZIP 导入边界规范化：忽略 `__MACOSX/**`、AppleDouble `._*` 与 `.DS_Store`；当且仅当全部真实文件共享唯一一层外目录、且剥离后能找到该 importer 指定的根 manifest 时，移除这层 Finder wrapper。四个编辑器的 package classifier 与 Symbols、Popup、image-string、layout、VNI 导入路径均复用该规则。规范化发生在 package 语义解析前，之后原有 canonical path、`assets.map.json`、完整 SHA-256、缺失文件与孤儿 payload 检查不变；元数据和 wrapper 不会进入 draft 或新导出。

补充门禁中，Game Layout 首次复跑暴露两处测试/类型问题：preview fixture 的 reel order 未启用 Symbols binding，wrapper key 顺序断言依赖 Map 迭代顺序；修正 fixture 与集合断言后通过。另一个旧测试要求非 ZIP 错误文本包含小写 `zip`，统一诊断大小写后通过。最终补充 scoped 结果为：`@slotclientengine/editorresource` 33 tests、`gamelayouteditor` 147 tests、`imgnumbereditor` 19 tests、`popupeditor` 14 tests、`symbolseditor` 44 tests，相关 lint/typecheck 均通过。四个 app 的 production build 均顺序通过；`editorresource` 与四个 app 的 `format:check` 全部通过，`git diff --check` exit 0。Vite 仍仅报告既有的大 chunk warning。真实浏览器验收继续由用户执行。

## 14. Game Layout 大写 filename key 导出补充（2026-07-22）

用户导出包含 `BG_2.webp` 的 layout 时遇到“package path 必须为小写”。该值是编辑器内部大小写敏感 filename key，不是最终 ZIP path；最终 payload 本应由顶层 `assets.map.json` 映射到 `assets/<full-lowercase-sha256>.webp`。根因是 Game Layout 导出器在 hash 物化前错误地对 filename key 调用了最终 package path 小写校验。

修复后，closure 阶段保留 filename key 的原始大小写；完成扁平 workspace、SHA-256 payload 和控制文件生成后，才对最终 ZIP entries 执行 canonical lowercase path 校验。新增 `BG_2.webp` 自有背景回归确认 manifest 与 map key 保留原名、payload path 为小写完整 hash、bytes 精确一致，并且 ZIP 内全部真实 entry 均为小写 canonical path。`gamelayouteditor` 最新为 `20 files / 148 tests passed`，lint、typecheck、production build 通过；首次 format gate 仅发现新测试排版，使用仓库 Prettier 格式化后复跑通过。真实浏览器导出验收仍由用户执行。

## 15. Game Layout 组合预览 CN value-presentation 补充（2026-07-22）

用户用 `/Users/zerro/Downloads/crave.zip` 复验时发现 Game Layout 组合预览仍看不到 CN。对真实 ZIP 的 `assets.map.json` 和映射 payload 逐项检查确认包本身完整：layout 的两个 mode 均绑定 `game002-s3/reels-001`；`gameconfig.json` 声明 `CN=8`、六列本地轮带包含 CN，并包含 `bgcoinweight`；symbol manifest 包含四档 CN Spine、共享 `Loop`、`coin` slot、image-string 数字闭包和 `defaultValues`。

根因在组合预览参数，而不是资源：独立 Symbols overlay 会调用 `setPresentationValue(defaultValues[0])`，但完整 layout 改走 `SceneLayoutPackageRuntime` 后，初始 `reels.main`、重新随机和 mode 切换只传了 scene/phase，漏传 `presentationValues`。CN 的显式 normal reel texture 是 transparent；value 为 `null` 时 tier Spine 不会创建，所以有 CN code 的格子呈现为空白。

修复后组合预览为每格生成 x-first `presentationValues`：value-presentation symbol 未配置 otherScene 时使用首个 default value；legacy presentation target 配置权重表或固定值时使用 assignment 覆盖；普通 symbol 保持 `null`。独立 Symbols preview 与组合 runtime 现在复用同一份已选/随机 scene，因此重新随机和映射修改会同步到实际组合 reel。新增 runtime-init 回归验证 `[25, null]` 数值矩阵精确传入。`gamelayouteditor` 最新为 `20 files / 149 tests passed`，lint、typecheck、production build、format 均通过，`git diff --check` exit 0；构建仅保留既有的大 chunk warning。真实浏览器显示仍由用户验收。
