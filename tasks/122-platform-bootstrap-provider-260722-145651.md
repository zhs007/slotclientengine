...
| chunk                           |         raw |   gzip -9 | 说明                                          |
| ------------------------------- | ----------: | --------: | --------------------------------------------- |
| `index-KZ9BuYgP.js`             |   182,262 B |  33,859 B | initial loading；无 React/Leo/Provider marker |
...
| `game-entry-NvRHKOjb.js`        | 1,560,731 B | 326,525 B | 100% 后 runtime、React、Leo HUD               |

initial 相比 Task 121 增加 1,298 B raw、172 B gzip。静态发布检查确认初始 chunk 无 Provider/React，bootstrap 与 runtime 各只有一个入口 URL，并共享同一个依赖 chunk，没有打两份等价代码。
...
新增 5 项 Provider 回归测试，并为 Leo Loading 动画几何补充断言；重新通过 gameloading-ui-leo 22/22 tests、platformbootstrap-leo 19/19 tests、game002 102/102 tests、lint、typecheck、format check、release check；game003 136/136 tests 与 release check 保持通过。
