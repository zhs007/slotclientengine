...
| chunk                           |         raw |   gzip -9 | 说明                                          |
| ------------------------------- | ----------: | --------: | --------------------------------------------- |
| `index-DL7YvR0R.js`             |   182,240 B |  33,837 B | initial loading；无 React/Leo/Provider marker |
...
| `game-entry-DcNOfAlC.js`        | 1,560,731 B | 326,525 B | 100% 后 runtime、React、Leo HUD               |

initial 相比 Task 121 增加 1,276 B raw、150 B gzip。静态发布检查确认初始 chunk 无 Provider/React，bootstrap 与 runtime 各只有一个入口 URL，并共享同一个依赖 chunk，没有打两份等价代码。
